...
The production-safe S0-S5 migration runner is implemented under `migrations/`. It uses an exact 46-file allowlist, reuses the canonical domain migration SQL, strips historical local-only credential lines and environment fixture seed blocks from the production execution stream, records the hash of the executed SQL in each database, and fails closed on migration drift.
