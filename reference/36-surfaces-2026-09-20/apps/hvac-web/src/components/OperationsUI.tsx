...
export function OperationsTimeline({ items }: { items: OperationsTimelineItem[] }) {
  return (
    <ol className="ops-detail-timeline">
      {items.map((item, index) => (
        <li className="ops-detail-timeline-item" key={index}>
          <span className="ops-detail-timeline-marker" aria-hidden="true" style={item.color ? { borderColor: item.color, color: item.color } : undefined}>{item.dot}</span>
          <div className="ops-detail-timeline-content">{item.children}</div>
        </li>
      ))}
    </ol>
  );
...
export function OperationsChartCard({
  ...
  return (
    <Card className={['ops-chart-card', className].filter(Boolean).join(' ')}>
      <CardHeader>
        <CardTitle>
      ...
      <CardContent>
        ...
        >
          {loading ? (
            <div className="ops-chart-state"><LoaderCircle className="size-5 animate-spin" aria-label="加载中" /></div>
          ) : empty ? (
            <div className="ops-chart-state text-sm text-muted-foreground">{emptyDescription}</div>
          ) : children}
        </div>
