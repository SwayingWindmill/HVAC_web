...
function Item({ asChild = false, className, variant, size, ...props }: React.ComponentProps<'div'> & VariantProps<typeof itemVariants> & { asChild?: boolean }) {
  const Component = asChild ? Slot.Root : 'div';
  return <Component data-slot="item" className={cn(itemVariants({ variant, size }), className)} {...props} />;
}
...
function ItemDescription({ className, ...props }: React.ComponentProps<'div'>) {
  return <div data-slot="item-description" className={cn('text-sm leading-relaxed text-muted-foreground', className)} {...props} />;
}
