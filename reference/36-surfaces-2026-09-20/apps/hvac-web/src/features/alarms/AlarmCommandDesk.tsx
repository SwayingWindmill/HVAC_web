...
const SEVERITY_PILLS: readonly DataTableViewPillOption[] = [
  { key: 'all', label: '全部告警', count: 5 },
  { key: 'CRITICAL', label: '紧急', count: 1 },
  { key: 'MAJOR', label: '重要', count: 2 },
  { key: 'WARNING', label: '一般', count: 2 },
];
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2.5">
            <h1 className="text-2xl font-bold tracking-tight text-foreground">告警中心</h1>
            <Badge variant="outline" className="text-xs font-normal">
              活动告警 3 件
            </Badge>
          </div>
          <p className="mt-1 text-sm text-muted-foreground">
            {site.displayName} · 实时监控设备告警与异常事件，支持消警处置与工单派发
          </p>
        </div>
        ...
        <Card className="shadow-xs">
          <CardHeader className="pb-3 border-b">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <CardTitle className="text-base font-semibold">当前告警列表</CardTitle>
              <DataTableViewPills
