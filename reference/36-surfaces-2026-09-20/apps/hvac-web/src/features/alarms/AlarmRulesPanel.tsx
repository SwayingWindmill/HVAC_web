...
function StatusBadge({ status }: { status: RuleExecutionEvidence['status'] }) {
  return <StatusPillBadge tone={statusTone(status)}>{statusLabel(status)}</StatusPillBadge>;
}
...
export function AlarmRulesPanel({ site, principal }: AlarmRulesPanelProps) {
  ...
  const evidenceColumns = useMemo<Array<ColumnDef<DataTableFeatures, RuleExecutionEvidence>>>(() => [
    { id: 'status', header: '判断状态', cell: ({ row }) => <StatusBadge status={row.original.status} /> },
    { id: 'result', header: '结果', cell: ({ row }) => <span className="text-xs">{row.original.status === 'SUCCEEDED' ? '规则判断已完成' : statusLabel(row.original.status)}</span> },
    { id: 'updatedAt', header: '更新时间', cell: ({ row }) => <span className="text-[11px] text-muted-foreground tabular-nums">{formatRuleTime(row.original.updatedAt, site.timezone)}</span> },
  ], [site.timezone]);

  const evidenceTable = useDataTable({
    key: `alarm-rule-evidence-${selectedBinding?.id ?? 'none'}`,
    data: selectedEvidence.slice(0, 8),
  ...
  const revisionColumns = useMemo<Array<ColumnDef<DataTableFeatures, RuleRevision>>>(() => [
    {
      ...
      cell: ({ row }) => (
        <span className="font-mono text-xs font-medium tabular-nums">
          ...
          {row.original.id === selectedRevision?.id ? <Badge variant="outline" className="ml-2">当前</Badge> : null}
        </span>
      ),
    },
    { id: 'status', header: '状态', cell: () => <StatusPillBadge tone="success">已发布</StatusPillBadge> },
    { id: 'note', header: '说明', cell: ({ row }) => <span className="text-xs text-muted-foreground">{row.original.id === selectedRevision?.id ? '当前站点正在使用' : '历史已发布版本'}</span> },
  ], [selectedRevision?.id]);

  const revisionTable = useDataTable({
    key: `alarm-rule-revisions-${selectedRevision?.ruleId ?? 'none'}`,
    data: relatedRevisions.slice(0, 10),
    ...
    return (
      <Card className="shadow-none">
        <CardHeader>
          ...
          <CardDescription>当前账号无权查看或管理规则版本、站点绑定和执行记录。告警中心不会用静态示例代替真实规则。</CardDescription>
  ...
  return (
    <div className="alarm-center__rules-workbench space-y-4" data-testid="alarm-rules-workbench">
      ...
      <div className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
        <Card className="shadow-none">
          <CardHeader><CardTitle>规则绑定</CardTitle><CardDescription>{orderedBindings.length} 条站点绑定</CardDescription></CardHeader>
            ...
            <>
              <Card className="shadow-none">
                ...
                <CardContent className="space-y-5">
                  ...
                  <div className="flex items-start gap-3 rounded-md border bg-muted/20 p-3"><Bell className="mt-0.5 size-4 shrink-0 text-muted-foreground" /><div><strong className="text-xs font-medium">通知配置以规则版本为准</strong><p className="mt-1 text-[11px] leading-5 text-muted-foreground">只有当前已发布规则中实际配置的通知对象和渠道才会显示；通知是否成功送达请在通知中心查看。</p></div><Button className="ml-auto shrink-0" variant="outline" size="xs" asChild><a href="/notifications">通知中心</a></Button></div>
                </CardContent>
              </Card>

              <Card className="shadow-none">
                <CardHeader><CardTitle className="flex items-center gap-2"><Link2 className="size-4 text-muted-foreground" />生效状态与执行记录</CardTitle><CardDescription>规则绑定与最近执行证据。</CardDescription><CardAction>{latestEvidence ? <StatusBadge status={latestEvidence.status} /> : <Badge variant="outline">暂无执行</Badge>}</CardAction></CardHeader>
                <CardContent className="space-y-4">
                  ...
                  {evidenceQuery.isPending ? <div className="flex min-h-28 items-center justify-center gap-2 text-xs text-muted-foreground"><LoaderCircle className="size-4 animate-spin" />正在读取执行记录</div>
                    : selectedEvidence.length > 0 ? (
                      <DataTable table={evidenceTable} tableAriaLabel="规则执行记录" />
                    ) : <div className="grid min-h-28 place-items-center text-xs text-muted-foreground">当前规则绑定没有执行记录</div>}
                </CardContent>
              </Card>

              <Card className="shadow-none">
                <CardHeader><CardTitle className="flex items-center gap-2"><History className="size-4 text-muted-foreground" />规则版本历史</CardTitle><CardDescription>已发布版本不可修改。</CardDescription></CardHeader>
                <CardContent className="p-0">
                  <DataTable table={revisionTable} className="gap-0" tableAriaLabel="规则版本历史" />
                </CardContent>
              </Card>
            </>
