...
import type { AssetsSnapshotResult } from './model.ts';

const MAX_BATCH_DEVICES = 100;
const MAX_BATCH_KEY_SELECTIONS = 2048;

export interface AssetsRegistryClient {
  getSiteAssetModel: PlatformGatewayClient['getSiteAssetModel'];
}

export interface AssetsRegistryData {
  readonly assetModel: SiteAssetModel;
  readonly routePolicyRevision: string | null;
}

export interface LoadAssetsRegistryInput {
  readonly client: AssetsRegistryClient;
  readonly tenantId: string;
  readonly siteId: string;
  readonly signal: AbortSignal;
}

export interface LoadAssetsCurrentStateInput {
...
export interface AssetsCurrentStateData {
  readonly byDeviceId: ReadonlyMap<string, AssetsSnapshotResult>;
...
export async function loadAssetsRegistry(input: LoadAssetsRegistryInput): Promise<AssetsRegistryData> {
...
export async function loadAssetsCurrentState(input: LoadAssetsCurrentStateInput): Promise<AssetsCurrentStateData> {
...
export function assetsRegistryQueryKey(
  generation: number,
  tenantId: string,
  siteId: string,
  routePolicyEpoch = 0,
...
export function assetsCurrentStateQueryKey(
  generation: number,
  tenantId: string,
  siteId: string,
  devices: readonly Device[],
