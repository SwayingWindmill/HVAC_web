...
import {
  ...
  type AssetsPointDefinition,
  type AssetsProfileResolution,
} from './catalog.ts';

export type AssetsHistoryRange = '1h' | '6h' | '24h';

export interface AssetsHistoryRangeDefinition {
  readonly label: string;
  readonly milliseconds: number;
  readonly pageSize: number;
}

export const REAL_ASSETS_HISTORY_RANGES: Readonly<Record<AssetsHistoryRange, AssetsHistoryRangeDefinition>> = Object.freeze({
...
export interface AssetsHistoryQuery {
  ...
  readonly range: AssetsHistoryRange;
...
export interface LoadAssetsHistoryInput {
  readonly client: S2TelemetryClient;
  readonly query: AssetsHistoryQuery;
  readonly sessionCapability: string;
  readonly signal: AbortSignal;
}

export interface AssetsTrendDatum {
...
export interface AssetsHistoryFailure {
...
export function listAssetsTrendDefinitions(profile: AssetsProfileResolution): readonly AssetsPointDefinition[] {
...
export function createAssetsHistoryQuery(input: {
  readonly protectedGeneration: number;
  readonly sessionId: string;
  readonly tenantId: string;
  readonly siteId: string;
...
export function assetsHistoryQueryKey(query: AssetsHistoryQuery): readonly unknown[] {
  return [
...
export function assetsHistoryRevisionKey(query: AssetsHistoryQuery, response: S2TelemetryContract.DeviceHistoryResponse): readonly unknown[] {
  return [
    ...assetsHistoryQueryKey(query),
    'projection', response.metadata.projectionWatermark ?? 'no-watermark', response.metadata.nextCursor ?? 'complete',
  ] as const;
}

export function validateAssetsHistoryResponse(response: S2TelemetryContract.DeviceHistoryResponse, query: AssetsHistoryQuery): S2TelemetryContract.DeviceHistoryResponse {
...
export async function loadAssetsHistory(input: LoadAssetsHistoryInput): Promise<S2TelemetryContract.DeviceHistoryResponse> {
  const options = {
  ...
  return validateAssetsHistoryResponse(response, input.query);
}
...
export function buildAssetsTrendData(
  observations: readonly DeviceHistoryObservation[],
  range: AssetsHistoryRange,
  pageSize: number,
): readonly AssetsTrendDatum[] {
...
export function formatAssetsHistoryInstant(value: string | number | null, timezone: string): string {
...
export function classifyAssetsHistoryFailure(error: unknown): AssetsHistoryFailure {
