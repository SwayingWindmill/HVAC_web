...
export type AssetsSnapshotResult =
  | { readonly status: 'ok'; readonly snapshot: DeviceObservationSnapshot }
  | { readonly status: 'error'; readonly problem: ProblemDetails };

export type AssetsConnectionState = DevicePresenceState | 'NOT_APPLICABLE' | 'UNAVAILABLE';
export type AssetsTelemetryFreshnessState = TelemetryFreshness | 'NOT_APPLICABLE' | 'UNAVAILABLE';
export type AssetsTelemetryQualityState = 'GOOD' | 'DEGRADED' | 'NO_DATA' | 'NOT_APPLICABLE' | 'UNAVAILABLE';
export type AssetsTelemetryReadinessState = TelemetryReadiness | 'UNAVAILABLE';

export type AssetsAttentionReason =
...
export interface AssetsPointView {
...
export interface AssetsConnectionProjection {
  readonly applicability: PresenceApplicability | 'UNAVAILABLE';
  readonly state: AssetsConnectionState;
  readonly lastSeenAt: string | null;
  readonly policyRevision: number | null;
}

export interface AssetsTelemetryProjection {
  readonly evaluationAvailability: EvaluationAvailability;
  readonly readiness: AssetsTelemetryReadinessState;
  readonly freshness: AssetsTelemetryFreshnessState;
  readonly quality: AssetsTelemetryQualityState;
...
export interface AssetsDeviceOperationalProjection {
  readonly connection: AssetsConnectionProjection;
  readonly telemetry: AssetsTelemetryProjection;
  readonly registryLifecycle: Device['status'];
  readonly points: readonly AssetsPointView[];
  readonly attentionReasons: readonly AssetsAttentionReason[];
  readonly needsAttention: boolean;
}

export interface ProjectAssetsDeviceOperationalStateInput {
...
function pointView(definition: PointDisplayDefinition, state: TelemetryKeyState | undefined): AssetsPointView {
...
function unavailablePointView(definition: PointDisplayDefinition): AssetsPointView {
  return {
    pointId: definition.point.id,
    sensorId: definition.point.sensorId ?? null,
    key: definition.point.pointCode,
...
function connectionFromSnapshot(snapshot: DeviceObservationSnapshot): AssetsConnectionProjection {
...
function aggregateFreshness(points: readonly AssetsPointView[]): TelemetryFreshness {
...
function aggregateQuality(points: readonly AssetsPointView[]): 'GOOD' | 'DEGRADED' | 'NO_DATA' {
...
function ownerFailureReason(problem: ProblemDetails): AssetsAttentionReason {
...
export function projectAssetsDeviceOperationalState(
  ...
  const attentionReasons = new Set<AssetsAttentionReason>();
