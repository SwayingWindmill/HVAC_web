...
export function RegistryResourceWorkbench({ site, model, capabilities, onDirtyChange, onRefresh, onSiteCreated }: Props) {
  ...
  const resourceColumns = useMemo<Array<ColumnDef<DataTableFeatures, ResourceRow>>>(() => [
    ...
    { id: 'subtype', header: '类型', cell: ({ row }) => row.original.subtype },
    {
      id: 'status',
      header: '状态',
      cell: ({ row }) => <StatusPillBadge tone={statusTone(row.original.status)}>{statusLabel(row.original.status)}</StatusPillBadge>,
    },
    {
      id: 'actions',
      header: '操作',
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
        ...
        <Card>
          <CardHeader>
            <CardTitle>登记资源</CardTitle>
            <CardDescription>创建、维护和退役正式登记对象；退役不会执行硬删除。</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {rows.length === 0 ? <div className="grid min-h-32 place-items-center rounded-md border border-dashed text-sm text-muted-foreground">当前没有{RESOURCE_LABEL[resourceType]}登记</div> : (
              <DataTable
                table={resourceTable}
      ...
      {selectedDeviceId ? (
        <Card>
          <CardHeader>
            <CardTitle>{selectedDevice?.displayName ?? '设备'} · 测点</CardTitle>
            <CardDescription>查看当前设备端点登记的测点与采集语义。</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex justify-end"><Button variant="ghost" size="sm" onClick={() => setSelectedDeviceId(null)}><X />关闭</Button></div>
            ...
            {pointsQuery.isLoading ? <div className="grid min-h-24 place-items-center text-sm text-muted-foreground">正在读取测点…</div> : devicePoints.length === 0 ? <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">当前设备没有测点登记</div> : (
              <DataTable
                table={pointTable}
                ...
                footer={(
                  <DataTablePagination
                    table={pointTable}
                    totalRows={devicePoints.length}
                  />
                )}
              />
            )}
            {pointsQuery.hasNextPage ? <Button variant="outline" onClick={() => void pointsQuery.fetchNextPage()} disabled={pointsQuery.isFetchingNextPage}><RefreshCw className={pointsQuery.isFetchingNextPage ? 'animate-spin' : undefined} />加载更多</Button> : null}
          </CardContent>
        </Card>
      ) : null}
        ...
        <DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-2xl">
          ...
          {editTarget ? (
            <div className="grid gap-4">
              ...
              {editTarget.type === 'POINT' ? <>
                <label className="grid gap-1.5 text-sm"><span>上报设备</span><Select value={editDraft.reportingDeviceId || undefined} onValueChange={(value) => updateEdit('reportingDeviceId', value)}><SelectTrigger><SelectValue placeholder="选择设备" /></SelectTrigger><SelectContent>{model.devices.map((device) => <SelectItem key={device.id} value={device.id}>{device.displayName} · {device.code}</SelectItem>)}</SelectContent></Select></label>
                <label className="grid gap-1.5 text-sm"><span>物理传感器（可选）</span><Select value={editDraft.sensorId || '__none__'} onValueChange={(value) => updateEdit('sensorId', value === '__none__' ? '' : value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="__none__">未绑定传感器</SelectItem>{model.sensors.map((sensor) => <SelectItem key={sensor.id} value={sensor.id}>{sensor.displayName} · {sensor.code}</SelectItem>)}</SelectContent></Select></label>
              ...
              <label className="grid gap-1.5 text-sm"><span>状态</span><Select value={editDraft.status} onValueChange={(value) => updateEdit('status', value as Site['status'])}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ACTIVE">启用</SelectItem><SelectItem value="INACTIVE">停用</SelectItem></SelectContent></Select></label>
      ...
      <Dialog open={rebindOpen} onOpenChange={(open) => { if (!open) closeRebind(); else setRebindOpen(true); }}>
        <DialogContent className="sm:max-w-xl">
          ...
          <div className="grid gap-4">
            <label className="grid gap-1.5 text-sm"><span>关系类型</span><Select value={rebindDraft.kind} onValueChange={(value) => updateRebind('kind', value as RebindKind)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="DEVICE_ASSET">设备端点 → 设备资产</SelectItem><SelectItem value="ASSET_SPACE">设备资产 → 空间</SelectItem><SelectItem value="DEVICE_SPACE">设备端点 → 空间</SelectItem><SelectItem value="POINT_SUBJECT">测点 → 业务对象</SelectItem></SelectContent></Select></label>
            {rebindDraft.kind === 'POINT_SUBJECT' ? <label className="grid gap-1.5 text-sm"><span>目标对象类型</span><Select value={rebindDraft.targetType} onValueChange={(value) => updateRebind('targetType', value as RebindDraft['targetType'])}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="SITE">站点</SelectItem><SelectItem value="SPACE">空间</SelectItem><SelectItem value="ASSET">设备资产</SelectItem></SelectContent></Select></label> : null}
            <label className="grid gap-1.5 text-sm"><span>来源对象</span><Select value={rebindDraft.sourceId || undefined} onValueChange={(value) => updateRebind('sourceId', value)}><SelectTrigger><SelectValue placeholder="选择来源对象" /></SelectTrigger><SelectContent>{bindingOptions.sources.map((option) => <SelectItem key={option.id} value={option.id}>{option.label}</SelectItem>)}</SelectContent></Select></label>
            <label className="grid gap-1.5 text-sm"><span>目标对象</span><Select value={rebindDraft.targetId || undefined} onValueChange={(value) => updateRebind('targetId', value)}><SelectTrigger><SelectValue placeholder="选择目标对象" /></SelectTrigger><SelectContent>{bindingOptions.targets.map((option) => <SelectItem key={option.id} value={option.id}>{option.label}</SelectItem>)}</SelectContent></Select></label>
            <label className="grid gap-1.5 text-sm"><span>关系角色</span><Select value={rebindDraft.role} onValueChange={(value) => updateRebind('role', value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{rebindRoles.map((role) => <SelectItem key={role} value={role}>{role}</SelectItem>)}</SelectContent></Select></label>
