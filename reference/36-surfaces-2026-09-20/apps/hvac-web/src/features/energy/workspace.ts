...
export interface EnergyWorkspaceSearchInput {
  readonly anchor?: string;
  readonly quality?: EnergyQualityPolicy;
}

export function energyWorkspaceStateFromRoute(
  period: EnergyWorkspacePeriod,
  search: EnergyWorkspaceSearchInput,
  timeZone: string,
  now = new Date(),
): EnergyWorkspaceState {
  const fallbackAnchor = currentDateInTimeZone(timeZone, now);
  const anchorCandidate = search.anchor ?? fallbackAnchor;
  const anchor = parseDateOnly(anchorCandidate) ? anchorCandidate : fallbackAnchor;
  return Object.freeze({
    period,
    anchor: dateOnly(periodStart(period, calendarDate(anchor))),
    qualityPolicy: search.quality ?? 'VALID_ONLY',
  });
}
