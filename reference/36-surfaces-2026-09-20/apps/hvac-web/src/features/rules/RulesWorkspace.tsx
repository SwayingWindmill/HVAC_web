...
export function RulesWorkspace({ siteId: _siteId }: RulesWorkspaceProps) {
  ...
  const severityPills: readonly DataTableViewPillOption[] = [
    { key: 'ALL', label: '全部级别', count: DETECTION_RULES.length },
    { key: 'CRITICAL', label: '紧急', count: DETECTION_RULES.filter((r) => r.severity === 'CRITICAL').length },
    { key: 'MAJOR', label: '重要', count: DETECTION_RULES.filter((r) => r.severity === 'MAJOR').length },
    { key: 'WARNING', label: '警告', count: DETECTION_RULES.filter((r) => r.severity === 'WARNING').length },
    { key: 'INFO', label: '提示', count: DETECTION_RULES.filter((r) => r.severity === 'INFO').length },
  ];

  const filteredRules = useMemo(() => {
    return DETECTION_RULES.filter((r) => {
  ...
  const columns = useMemo<Array<ColumnDef<DataTableFeatures, DetectionRule>>>(() => [
    ...
    { id: 'code', header: '规则代码', cell: ({ row }) => <span className="font-mono font-semibold text-foreground">{row.original.code}</span> },
    {
      id: 'name',
      header: '规则名称与监控对象',
    ...
    { id: 'suppression', header: '防雪崩窗口', cell: ({ row }) => <span className="font-mono text-[11px] text-muted-foreground tabular-nums">{row.original.suppressionWindowMin} 分钟</span> },
    { id: 'enabled', header: '状态', cell: ({ row }) => <StatusPillBadge tone={row.original.enabled ? 'success' : 'neutral'} label={row.original.enabled ? '已启用' : '已停用'} /> },
    {
      id: 'actions',
      header: '操作',
      cell: ({ row }) => (
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between border-b pb-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold tracking-tight text-foreground">报警规则与通知</h1>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            设置设备异常触发条件、恢复死区与通知方式，规范报警推送并防止频繁刷屏
          </p>
        </div>
        <div className="flex items-center gap-2">
