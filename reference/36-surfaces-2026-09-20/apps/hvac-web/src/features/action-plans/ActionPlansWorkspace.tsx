...
export function ActionPlansWorkspace({ siteId: _siteId }: ActionPlansWorkspaceProps) {
  ...
  const statusPills: readonly DataTableViewPillOption[] = [
    { key: 'ALL', label: '全部状态', count: ACTION_PLANS.length },
    { key: 'IN_PROGRESS', label: '执行中', count: ACTION_PLANS.filter((p) => p.status === 'IN_PROGRESS').length },
    { key: 'COMPLETED', label: '已结项', count: ACTION_PLANS.filter((p) => p.status === 'COMPLETED').length },
    { key: 'BLOCKED', label: '受阻挂起', count: ACTION_PLANS.filter((p) => p.status === 'BLOCKED').length },
  ];

  const filteredPlans = useMemo(() => {
    return ACTION_PLANS.filter((p) => {
      ...
      return (
        ...
        p.seuReference.toLowerCase().includes(q)
      );
    });
  ...
  const columns = useMemo<Array<ColumnDef<DataTableFeatures, ActionPlan>>>(() => [
    ...
    { id: 'code', header: '计划编号', cell: ({ row }) => <span className="font-mono font-semibold text-foreground">{row.original.code}</span> },
    {
      id: 'title',
      header: '行动计划名称与归属',
    ...
    { id: 'period', header: '工期周期', cell: ({ row }) => <span className="font-mono text-[11px] text-muted-foreground tabular-nums">{row.original.period}</span> },
    {
      id: 'budget',
      header: '投资预算',
  ...
  return (
    <Main className="space-y-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between border-b pb-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold tracking-tight text-foreground">目标与行动计划</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">年度节电目标达成率</CardTitle>
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">累计核验节费贡献</CardTitle>
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">行动计划推进态势</CardTitle>
          ...
          <CardContent className="space-y-1.5">
            ...
            <div className="flex items-center justify-between text-[11px]">
              <span className="text-muted-foreground">纳管 SEU</span>
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">实施受阻风险预警</CardTitle>
      ...
      {/* 3. Action Plans Ledger Table Container */}
      <Card className="shadow-xs">
        {/* Filter Toolbar */}
        <CardHeader className="p-4 border-b space-y-3">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <CardTitle className="text-base font-semibold">节能行动计划列表</CardTitle>
              <p className="text-xs text-muted-foreground mt-0.5">
                跟踪各项节能措施的工期、预算、验收进度与实际节电核算
              </p>
            </div>
          ...
          <DataTable
            ...
            getHeaderCellProps={(header) => ({
              className:
                ...
                header.id === 'owner' || header.id === 'period' ? 'w-[140px]' :
