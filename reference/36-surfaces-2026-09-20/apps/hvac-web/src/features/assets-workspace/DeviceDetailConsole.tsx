...
export function DeviceDetailConsole({ onBack }: DeviceDetailConsoleProps) {
  type ThermalParameterRow = (typeof THERMAL_PARAMETER_ROWS)[number];

  const thermalColumns = useMemo<Array<ColumnDef<DataTableFeatures, ThermalParameterRow>>>(() => [
    { id: 'name', header: '物理参数', cell: ({ row }) => <span className="font-medium text-foreground">{row.original.name}</span> },
    { id: 'value', header: '当前实测值', cell: ({ row }) => <span className="font-mono font-semibold text-foreground tabular-nums">{row.original.val}</span> },
    { id: 'limit', header: '设计安全区间', cell: ({ row }) => <span className="font-mono text-muted-foreground tabular-nums">{row.original.limit}</span> },
    { id: 'status', header: '工况状态', cell: () => <StatusPillBadge tone="success">正常</StatusPillBadge> },
  ], []);

  const thermalTable = useDataTable({
    key: 'device-detail-thermal-parameters',
    data: [...THERMAL_PARAMETER_ROWS],
        ...
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <div>
            ...
            <CardDescription className="text-xs">
              主机轴功率 (左轴 kW) 与 综合制冷能效 COP (右轴) 同步走势
            </CardDescription>
          </div>
