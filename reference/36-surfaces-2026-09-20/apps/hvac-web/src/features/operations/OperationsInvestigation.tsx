...
import {
  OperationsInvestigationStream,
  type OperationsInvestigationConnectionState,
} from './OperationsInvestigationStream';
  ...
  return (
    <nav className="operations-list" aria-labelledby="operations-list-title">
      ...
      {investigations.length === 0 ? (
        <p className="operations-muted">当前站点暂无排查记录。</p>
      ) : (
        <ul>
  ...
  return (
    <li className="operations-source">
      ...
      <dl className="operations-provenance">
        <div><dt>请求追踪</dt><dd>{source.requestId}</dd></div>
        <div><dt>设备台账版本</dt><dd>{source.registryRevision ?? '—'}</dd></div>
        <div><dt>数据集版本</dt><dd>{source.datasetRevision ?? '—'}</dd></div>
        <div><dt>数据水位线</dt><dd>{source.watermark.data ?? '—'}</dd></div>
        <div><dt>聚合水位线</dt><dd>{source.watermark.aggregate ?? '—'}</dd></div>
        <div><dt>数据完整性</dt><dd>{source.partial ? '部分有效' : '完整'}</dd></div>
        <div>
          <dt>数据质量分布</dt>
          <dd>{source.quality.valid} 有效 · {source.quality.suspect} 可疑 · {source.quality.invalid} 无效</dd>
        </div>
        <div><dt>采样时间</dt><dd>{formatTimestamp(source.capturedAt, timezone)}</dd></div>
        <div><dt>评估时间</dt><dd>{formatTimestamp(source.evaluatedAt, timezone)}</dd></div>
        <div><dt>数据指纹</dt><dd className="operations-digest">{source.provenanceDigest}</dd></div>
      </dl>
    </li>
  );
}
  ...
  return (
    ...
    >
      <section className="operations-summary">
        <div>
          <p className="real-shell-eyebrow">排查任务视图</p>
          <h2 id="operations-plan-title">{snapshot.plan.label}</h2>
          <p>
            版本 v{investigation.revision} · {statusLabel(investigation.status, investigation.outcome)}
          </p>
        </div>
      ...
      <section aria-labelledby="operations-plan-steps-title">
        <h3 id="operations-plan-steps-title">排查执行步骤</h3>
      ...
      <section aria-labelledby="operations-evidence-title">
        <div className="operations-section-heading">
          <h3 id="operations-evidence-title">排查证据</h3>
          <strong>{investigation.evidence.length}</strong>
        </div>
        {investigation.evidence.length === 0 ? <p className="operations-muted">暂无已提交证据。</p> : (
      ...
      <section aria-labelledby="operations-analysis-title">
        <div className="operations-section-heading">
          <h3 id="operations-analysis-title">分析引用</h3>
          <strong>{investigation.analysisReferences.length}</strong>
        </div>
        {investigation.analysisReferences.length === 0 ? <p className="operations-muted">暂无分析引用记录。</p> : (
          <div className="operations-card-list">
      ...
      <section aria-labelledby="operations-findings-title">
        <div className="operations-section-heading">
          <h3 id="operations-findings-title">诊断发现</h3>
          <strong>{investigation.findings.length}</strong>
        </div>
        {investigation.findings.length === 0 ? <p className="operations-muted">暂无诊断发现记录。</p> : (
          <div className="operations-card-list">
      ...
      <section aria-labelledby="operations-tools-title">
        <div className="operations-section-heading">
          <div>
            <h3 id="operations-tools-title">工具执行记录</h3>
            <p className="operations-muted">显示已执行工具调用的审计凭据与执行摘要。</p>
          </div>
          <strong>{toolReceipts.length}</strong>
        </div>
        {toolReceipts.length === 0 ? <p className="operations-muted">当前版本暂无工具执行凭证。</p> : (
...
export function OperationsInvestigation({
  ...
  const [investigationId, setInvestigationId] = useState(() => investigationFromLocation(location.searchStr));
  const [openValue, setOpenValue] = useState(() => investigationFromLocation(location.searchStr));
  ...
  const openInvestigation = useCallback((nextId: string) => {
    ...
    void router.navigate({ href: `${location.pathname}?${parameters.toString()}${location.hash}` });
  ...
  useEffect(() => {
    const next = investigationFromLocation(location.searchStr);
    setInvestigationId(next);
    setOpenValue(next);
    setSnapshot(null);
    setToolReceipts([]);
  ...
  const stream = useMemo(() => investigationId ? new OperationsInvestigationStream({
  ...
  useEffect(() => {
    ...
    void stream.start().catch((error: unknown) => {
      if (mounted) setFailure(error instanceof Error ? error : new Error(String(error)));
    });
    return () => {
      mounted = false;
      stream.abort();
  ...
  }, [stream, investigationId, registerProtectedResource, site.id]);
  ...
  return (
    ...
    >
      <header className={embedded ? 'operations-header operations-header--embedded' : 'operations-header'}>
        {!embedded ? (
          <div>
            <p className="real-shell-eyebrow">运行排查工作台</p>
            <h1>站点运行排查</h1>
            <p>站点 {site.displayName} · 运行异常排查、执行计划与现场数据核实记录。</p>
          </div>
        ) : <span />}
        ...
        >
          ...
          <span>
            {connection.status === 'RETRYING'
              ? '连接中断，正在重新授权并从有效位置恢复；不会触发新的业务写入。'
      ...
      <div className="operations-layout">
        <aside>
          {listLoading ? (
            <div className="real-shell-progress" role="status" aria-live="polite">正在读取排查任务列表…</div>
          ) : listFailure ? (
            <div className="real-shell-problem" role="alert">
              <strong>排查任务列表暂不可用</strong>
