...
export function DataQualityWorkspace({ siteId: _siteId }: DataQualityWorkspaceProps) {
  ...
  const statusPills: readonly DataTableViewPillOption[] = [
    { key: 'ALL', label: '全部点位', count: TELEMETRY_POINTS.length },
    { key: 'ISSUES', label: '存疑/异常', count: TELEMETRY_POINTS.filter((p) => p.status !== 'GOOD').length },
    { key: 'GOOD', label: '正常有效', count: TELEMETRY_POINTS.filter((p) => p.status === 'GOOD').length },
    { key: 'SUSPECT', label: '数据存疑', count: TELEMETRY_POINTS.filter((p) => p.status === 'SUSPECT').length },
    { key: 'OUT_OF_RANGE', label: '越限超标', count: TELEMETRY_POINTS.filter((p) => p.status === 'OUT_OF_RANGE').length },
    { key: 'STALE', label: '冻结/离线', count: TELEMETRY_POINTS.filter((p) => p.status === 'STALE' || p.status === 'OFFLINE').length },
  ];

  const filteredPoints = useMemo(() => {
    return TELEMETRY_POINTS.filter((pt) => {
      if (statusFilter === 'ISSUES' && pt.status === 'GOOD') return false;
      if (statusFilter === 'SUSPECT' && pt.status !== 'SUSPECT') return false;
      if (statusFilter === 'OUT_OF_RANGE' && pt.status !== 'OUT_OF_RANGE') return false;
      if (statusFilter === 'STALE' && pt.status !== 'STALE' && pt.status !== 'OFFLINE') return false;
      if (statusFilter === 'GOOD' && pt.status !== 'GOOD') return false;

      if (subsystemFilter !== 'ALL' && pt.subsystem !== subsystemFilter) return false;
      ...
      return (
        ...
        pt.deviceName.toLowerCase().includes(q) ||
        pt.subsystem.toLowerCase().includes(q)
      );
    });
  }, [statusFilter, subsystemFilter, searchQuery]);
  ...
  return (
    <Main className="space-y-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between border-b pb-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold tracking-tight text-foreground">数据质量</h1>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            监控全站传感器采集状态，排查异常数据对能效统计和控制策略的影响
          </p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">遥测点位在线覆盖率</CardTitle>
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">P95 遥测传输时延</CardTitle>
      ...
      {/* 4. Telemetry Points Ledger Table Container */}
      <Card className="shadow-xs">
        {/* Filter Toolbar */}
        <div className="p-4 border-b flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between bg-muted/10">
          <div>
            <h3 className="text-sm font-semibold text-foreground">测点运行与数据质量列表</h3>
            <p className="text-xs text-muted-foreground">
              查看各子系统传感器读数、采样频率与通信质量
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              <option value="冷冻水系统">冷冻水系统</option>
              <option value="冷却水系统">冷却水系统</option>
              <option value="水泵输配系统">水泵输配系统</option>
              <option value="电能计量系统">电能计量系统</option>
            </select>
