...
export function ControlOptimizationConsole({ site }: ControlOptimizationConsoleProps) {
  ...
  const dispatchColumns = useMemo<Array<ColumnDef<DataTableFeatures, DispatchHistoryItem>>>(() => [
    ...
    { id: 'status', header: '执行状态', cell: ({ row }) => <StatusPillBadge tone={row.original.ok ? 'success' : 'destructive'} label={row.original.status} /> },
  ], []);
    ...
    <Main className="space-y-6" data-testid="control-optimization-console">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2.5">
            <h1 className="text-2xl font-bold tracking-tight text-foreground">控制中心</h1>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {site.displayName} · {site.timezone} · 安全联锁闭环受控 · 寻优控制指令调度
          </p>
        </div>
      ...
      <Card className="shadow-xs">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-base font-semibold">控制指令与参数变更审计日志</CardTitle>
              <CardDescription className="text-xs">
                追踪记录自动优化指令与人工接管操作
              </CardDescription>
            </div>
        ...
        <CardContent className="p-0">
          <DataTable
            table={dispatchTable}
            className="gap-0"
            tableAriaLabel="控制指令与参数变更审计日志"
            getHeaderRowProps={() => ({ className: 'bg-muted/20 hover:bg-transparent' })}
            getHeaderCellProps={(header) => ({
              className:
                header.id === 'select' ? 'w-12 text-center' :
                header.id === 'time' ? 'w-28 text-xs' :
                header.id === 'from' || header.id === 'to' ? 'text-right text-xs' :
            ...
            footer={(
              <DataTablePagination
                table={dispatchTable}
                totalRows={DISPATCH_HISTORY.length}
              />
            )}
          />
        </CardContent>
