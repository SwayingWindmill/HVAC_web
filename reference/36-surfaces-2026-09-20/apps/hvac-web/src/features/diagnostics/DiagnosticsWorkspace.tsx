...
import { siteAssetDeviceRoute, siteRoute } from '@/app/router-paths';
...
function sourceExplanation(finding: FDDFinding): string {
  if (finding.ruleRevisionId) {
    return '由预设诊断规则计算触发，建议结合现场实际运行工况与测点历史复核。';
  }
  if (finding.modelDeploymentRevisionId) {
    return '基于设备运行数据与算法模型分析生成，置信度仅供参考，建议现场排查确认。';
  }
...
export function DiagnosticsWorkspace({
  ...
  const sourcePills: readonly DataTableViewPillOption[] = useMemo(() => [
    { key: 'ALL', label: '全部来源', count: rows.length },
    { key: '规则结果', label: '规则结果', count: rows.filter((r) => resultSource(r.finding) === '规则结果').length },
    { key: '模型结果', label: '模型结果', count: rows.filter((r) => resultSource(r.finding) === '模型结果').length },
    { key: '已发布结果', label: '已发布结果', count: rows.filter((r) => resultSource(r.finding) === '已发布结果').length },
  ], [rows]);
  ...
  const columns = useMemo<Array<ColumnDef<DataTableFeatures, DiagnosisRow>>>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <StatusPillBadge label={src} tone={tone} />;
      },
    },
    {
      id: 'confidence',
    ...
    >
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          ...
          <h1 className="mt-1 text-2xl font-semibold tracking-tight">诊断中心</h1>
          <p className="mt-1 text-sm text-muted-foreground">基于设备运行数据与专家规则排查故障原因，提供处置建议并支持转派工单。</p>
        </div>
      ...
      <div className="rounded-md border bg-muted/20 p-3 text-xs leading-5 text-muted-foreground">
        <strong className="text-foreground">诊断提示：</strong> 系统根据现场测点数据与规则引擎输出诊断结论；若缺乏关键实测数据，需现场核实后再行处置。
      </div>
      ...
      {sourceContext ? (
        <div className="flex flex-col gap-2 rounded-md border bg-card p-3 text-xs sm:flex-row sm:items-center sm:justify-between" data-testid="diagnostics-source-context">
          <div className="flex min-w-0 items-center gap-2"><Link2 className="size-3.5 text-muted-foreground" /><span>来自告警中心 · {rows.some((row) => row.finding.alarmId === searchState.alarm) ? '已找到显式关联的诊断结果' : '当前没有显式关联的已发布诊断结果'}</span></div>
      ...
      {findingsQuery.isError ? (
        ...
        <div className="grid min-h-[420px] place-items-center rounded-lg border border-dashed text-center">
          <div className="max-w-md px-6">
            ...
            <p className="mt-1 text-xs leading-5 text-muted-foreground">当前各系统运行数据未触发故障诊断规则，运行状态良好。</p>
          </div>
        </div>
      ) : (
        <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(420px,0.9fr)_minmax(0,1.5fr)]">
          <Card className="min-w-0 gap-0 py-0 shadow-none" data-testid="diagnostics-queue">
            <CardHeader className="border-b py-4">
              <CardTitle>诊断结果队列</CardTitle>
              <CardDescription>共 {filteredRows.length} 项诊断结果</CardDescription>
            </CardHeader>
              ...
              <div className="border-b p-3 space-y-2.5">
                ...
                <div className="overflow-x-auto">
                  <DataTableViewPills
                    options={sourcePills}
                    value={sourceFilter}
                    onValueChange={(val) => onSearchChange({ source: val === 'ALL' ? undefined : val })}
                  />
                </div>
              </div>
              {filteredRows.length === 0 ? (
                <div className="grid min-h-72 place-items-center px-6 text-center text-xs text-muted-foreground">当前搜索条件下没有诊断结果</div>
              ) : (
                <DataTable
                  table={table}
                  tableAriaLabel="诊断结果队列"
                  className="gap-0"
                  getHeaderCellProps={() => ({ className: 'h-10 text-[11px]' })}
                  getRowProps={(row) => ({
            ...
            <div className="min-w-0 space-y-4" data-testid="diagnostics-detail">
              ...
              <section className="rounded-lg border bg-card p-4" aria-labelledby="diagnostics-hypothesis-title">
                ...
                <div className="mt-3 rounded-md border border-dashed p-4">
                  <Badge variant="secondary">尚无明确根因</Badge>
              ...
              <section className="rounded-lg border bg-card p-4" aria-labelledby="diagnostics-impact-title">
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-md border p-3"><span className="text-[11px] text-muted-foreground">关联资产</span><strong className="mt-1 block text-xs font-medium">{selected.assetLabel}</strong><Button className="mt-2 px-0" variant="link" size="sm" asChild><a href={`${siteRoute(site, 'assets')}?q=${encodeURIComponent(selected.assetLabel)}`}>在设备中心查看 <ArrowRight /></a></Button></div>
                  <div className="rounded-md border p-3"><span className="text-[11px] text-muted-foreground">关联设备</span><strong className="mt-1 block text-xs font-medium">{selected.deviceLabels.length > 0 ? selected.deviceLabels.slice(0, 3).join('、') : '暂无可展示关联设备'}</strong>{selected.deviceLabels.length > 3 ? <small className="mt-1 block text-[10px] text-muted-foreground">另有 {selected.deviceLabels.length - 3} 台</small> : null}{singleDeviceId ? <Button className="mt-2 px-0" variant="link" size="sm" asChild><a href={siteAssetDeviceRoute(site, singleDeviceId)}>打开设备详情 <ArrowRight /></a></Button> : null}</div>
                </div>
              ...
              <section className="rounded-lg border bg-card p-4" aria-labelledby="diagnostics-next-title">
                ...
                <ol className="mt-3 list-decimal space-y-2 pl-5 text-xs leading-5 text-muted-foreground">
                  ...
                  {selected.deviceIds.length > 0 ? <li>检查关联设备实时数据与运行趋势，确认故障是否依然存在。</li> : <li>当前诊断未直接关联单台设备，可在设备中心查看系统概况。</li>}
                  {selectedFinding.workOrderId ? <li>核对关联工单处理进展与责任人。</li> : <li>若需现场检修核实，可转派工单跟进。</li>}
                </ol>
                {!selectedFinding.workOrderId ? <Button className="mt-3" variant="outline" size="sm" asChild><a href={siteRoute(site, 'work-orders')}><ClipboardList />进入工单中心</a></Button> : null}
              </section>
