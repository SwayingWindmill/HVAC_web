...
export function LinkageAnalysisPage({ devices, summary, overview, selectedDeviceId, onOpenDevice, onOpenPlant, onOpenTerminal, onOpenAnomaly, onOpenModes }: {
  ...
  const linkageColumns = useMemo<Array<ColumnDef<DataTableFeatures, MonitorDevice>>>(() => [
    {
      id: 'device',
      header: '设备',
      cell: ({ row }) => <Button variant="link" className="h-auto p-0" onClick={() => onOpenDevice(row.original.device.id)}>{row.original.device.displayName}</Button>,
    },
    {
      id: 'state',
      header: '运行状态',
      cell: ({ row }) => <Badge variant="outline" className={deviceStateBadgeClass(row.original.state)}>{deviceStateLabel(row.original.state)}</Badge>,
    },
    {
      id: 'metrics',
      header: '关键参数',
      cell: ({ row }) => row.original.state.points.filter((point) => point.state === 'PRESENT').slice(0, 3).map((point) => `${point.label} ${point.displayValue}${point.unit ?? ''}`).join(' · ') || '—',
    },
    {
      id: 'analysis',
      header: '分析线索',
      cell: ({ row }) => row.original.activeAlarms.length
        ? <Button variant="link" className="h-auto p-0" onClick={() => onOpenAnomaly(row.original.activeAlarms[0]!.alarmId)}>存在活动异常</Button>
        : '暂无异常线索',
    },
  ], [onOpenAnomaly, onOpenDevice]);

  const linkageTable = useDataTable({
    key: 'monitor-linkage-device-table',
    data: [...plant],
  ...
  return (
    <div className="hvac-monitor__deep-page">
      ...
      >
        <DataTable
          table={linkageTable}
          tableAriaLabel="设备联动表格"
          getHeaderCellProps={(header) => ({
            className:
              header.id === 'device' ? 'min-w-44' :
              header.id === 'state' ? 'w-32' :
              header.id === 'metrics' ? 'min-w-64' :
              'w-36',
          })}
          getRowProps={(row) => ({
            className: row.original.device.id === selectedDeviceId ? 'hvac-monitor__linkage-row is-selected' : 'hvac-monitor__linkage-row',
            onClick: (event) => {
              const target = event.target as HTMLElement;
              if (target.closest('button,a')) return;
              onOpenDevice(row.original.device.id);
            },
          })}
        />
      </MonitorPanel>
