...
| Centrifugo `v6.8.1` | Authenticated WebSocket connection, subscription, live publication, one offline publication recovery, retained history and Prometheus metrics; supplemental S2 experiment adds platform subscribe authorization, live revocation, Snapshot/Revision reconciliation, retention/restart fallback, bounded fan-out and slow-consumer handling | Passed locally with bounded responsibility | Adopt candidate for S2 connection/publication transport only. Platform ownership of authorization, Snapshot, business Revision, fallback and audit is mandatory; see `docs/research/s2-realtime-transport-evaluation.md`. |
