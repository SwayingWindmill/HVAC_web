...
const expectedDomains = [
  ...
  'Energy Topology', 'Metric', 'Energy Processing', 'Energy Content', 'Tariff', 'Settlement', 'Alarm', 'Control', 'Config',
...
invariant(JSON.stringify(baseline.logicalDomains.map(({ domain }) => domain)) === JSON.stringify(expectedDomains), 'logical-domain inventory drifted');
...
const contractOnlyGateway = await readFile(resolve(root, 'cmd/energy-api/internal/gateway/v212_contract_only.go'), 'utf8');
const contractOnlyRoutes = [
  ['GET', '/api/v1/devices'],
  ['POST', '/api/v1/telemetry/latest'],
  ['GET', '/api/v1/telemetry/history'],
];
for (const [method, path] of contractOnlyRoutes) {
  const operation = publicOpenAPI.paths?.[path]?.[method.toLowerCase()];
  invariant(contractOnlyGateway.includes(`template: "${path}"`), `contract-only runtime route is missing: ${method} ${path}`);
  invariant(operation?.['x-architecture-status'] === 'CONTRACTED' && operation?.['x-shape-status'] === 'TO_SYNC_TO_SE_API_001', `contract-only machine route drifted: ${method} ${path}`);
}
...
const commandGateway = await readFile(resolve(root, 'cmd/energy-api/internal/gateway/command.go'), 'utf8');
...
const gatewayServer = await readFile(resolve(root, 'cmd/energy-api/internal/gateway/server.go'), 'utf8');
...
const commandService = await readFile(resolve(root, 'modules/command/pkg/commandservice/http.go'), 'utf8');
...
invariant(optimizationService.includes('type SolverAdapter interface') && optimizationService.includes('Reconcile('), 'Optimization solver/reconciliation boundary is incomplete');
...
const supportDomains = await readFile(resolve(root, 'infra/registry/postgres/init/009j-operations-support-domains-v2.sql'), 'utf8');
...
const mlopsMetadata = await readFile(resolve(root, 'infra/registry/postgres/init/009k-mlops-metadata-v2.sql'), 'utf8');
...
invariant(phase1MigrationPaths.includes('infra/registry/postgres/init/009d-data-governance-v2.sql'), 'Phase1 must deploy Data Governance before Object Storage governance');
invariant(phase1MigrationPaths.includes('infra/registry/postgres/init/009g-object-storage-governance-v2.sql'), 'Phase1 must deploy Object Storage governance metadata');
...
const embeddedEnergy = await readFile(resolve(root, 'cmd/energy-api/embedded_energy.go'), 'utf8');
