...
const [model, postgres, scheduler, http, migration, alarmOutbox, alarmPostgres, alarmEvaluator, alarmMigration, gateway, gatewayServer, ownership, openapi] = await Promise.all([
  ...
  read('services/alarm-service/pkg/alarmservice/notification_outbox.go'),
  read('services/alarm-service/pkg/alarmservice/postgres.go'),
  read('services/alarm-service/pkg/alarmservice/evaluator_postgres.go'),
  read('services/alarm-service/migrations/007_s16_notification_outbox.sql'),
  read('services/platform-gateway/internal/gateway/notification.go'),
