...
const [routes, data, storeRead, storeMutation, cursor, http, main, migration001, migration002, migration003, roles, seed, compose, runner, workflow] = await Promise.all([
  ...
  readText('modules/workorder/pkg/workorderservice/postgres.go'),
  readText('modules/workorder/pkg/workorderservice/postgres_mutation.go'),
  readText('modules/workorder/pkg/workorderservice/cursor.go'),
  ...
  readText('modules/workorder/testdata/postgres/010_seed.sql'),
...
assert(store.includes('openWorkOrderPool(ctx, databaseURL, "s5_work_order_service")') && store.includes('openWorkOrderPool(ctx, mutationDatabaseURL, "s5_work_order_mutation_service")'), 'Work Order Store does not require distinct read and mutation login identities');
assert(store.includes('SET LOCAL ROLE s5_work_order_runtime'), 'Work Order Store does not explicitly activate the read-only runtime role');
...
assert(store.includes('mutationPool') && store.includes('s5_work_order_mutation_service') && store.includes('s5_work_order_writer') && store.includes('work_order_idempotency') && store.includes('work_order_mutation_audit'), 'Work Order Store lacks isolated durable mutation authority');
assert(!store.includes('DELETE FROM work_order_runtime'), 'Work Order Store contains delete authority');
...
assert(!migration.includes('GRANT DELETE') && !migration.includes('GRANT ALL'), 'Work Order writer received delete or all privileges');
