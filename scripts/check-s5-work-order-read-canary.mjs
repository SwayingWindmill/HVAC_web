...
const routes = registry.routes.filter((route) => route.owner === 'work-order-service');
assert(routes.length === 2, 'Work Order must expose exactly the list/detail route pair');
...
assert(!registry.routes.some((route) => route.owner === 'work-order-service' && route.method !== 'GET'), 'Work Order lifecycle route was introduced');
...
assert(openapi.includes('"const": 4') && openapi.includes('"work-order.list"') && openapi.includes('"work-order.read"') && openapi.includes('"maxItems": 16'), 'public capability v4 contract is incomplete');
for (const marker of ['CapabilitySetVersion = 4', 'CapabilityWorkOrderList', 'CapabilityWorkOrderRead']) assert(capabilities.includes(marker), `identity capability contract is missing ${marker}`);
