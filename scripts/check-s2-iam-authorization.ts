...
const [
  ...
  readText('infra/registry/postgres/init/006-s2-telemetry-authorization.sql'),
  readText('libs/telemetryauth/scope.go'),
  readText('libs/telemetryauth/grant.go'),
  readText('modules/iam/internal/iam/telemetry_authorization.go'),
  readText('modules/iam/internal/iam/postgres_telemetry_authorization.go'),
  readText('modules/iam/internal/iam/server.go'),
  readText('modules/iam/internal/iam/telemetry_server.go'),
  readText('modules/iam/internal/iam/telemetry_runtime_server.go'),
  readText('modules/iam/internal/iam/telemetry_grant_store.go'),
  readText('modules/iam/internal/iam/telemetry_audit.go'),
  readText('libs/telemetryauth/telemetry_test.go'),
  readText('modules/iam/internal/iam/telemetry_authorization_test.go'),
  readText('modules/iam/internal/iam/telemetry_server_test.go'),
  readText('modules/iam/internal/iam/telemetry_grant_store_integration_test.go'),
  readText('modules/iam/cmd/iam-owner/main.go'),
...
for (const binding of ['initiatingPrincipal', 'subjectIssuer', 'subject', 'tenantId', 'actorChain', 'action', 'scopeDigest', 'targetCount', 'keyCount', 'policyRevision', 'sessionId', 'parentTokenId', 'requestId', 'traceId', 'publicRoute', 'issuedAt', 'expiresAt', 'tokenId']) {
...
assert(s2Routes.length === 4, 'exactly four S2 runtime public routes must remain registered');
...
includesAll(authorizationTests, ['TestTelemetryAuthorizationRequiresExactDeviceAndKeyScope', 'TestTelemetryAuthorizationDoesNotTreatTenantMembershipAsAllSites', 'TestTelemetryAuthorizationRejectsCrossTenantDevice', 'TestTelemetryAuthorizationDenyPrecedenceAndStableKeyFailure', 'TestTelemetryAuthorizationIsAllOrNothing', 'TestPostgresTelemetryActionProjectionRejectsUnknownSchemaDrift'], 'authorization tests');
...
assert(packageJSON.scripts?.['s2:iam:check'] === 'node --experimental-strip-types scripts/check-s2-iam-authorization.ts', 's2:iam:check is not wired');
...
for (const command of ['npm run s2:iam:check', 'npm run s2:iam:postgres', 'test ./libs/telemetryauth/...', 'test ./modules/iam/...', 'npm run ownership:check', 'npm run lint', 'npm run build']) {
