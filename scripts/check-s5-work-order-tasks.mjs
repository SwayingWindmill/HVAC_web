...
for (const marker of ['exact-snapshot-replay', 'dual-version-conflict', 'unified-task-idempotency-domain', 'exact-full-permutation', 'authorization-denial-no-data', 'non-selected-route-absence', 'delete-and-title-edit-absence']) assert(browser.includes(marker), `P6 browser audit marker missing: ${marker}`);
