...
for (const marker of ["apps/hvac-web/vite.config.ts", "S0_GATEWAY_ONLY: 'true'", 'authenticated-principal-status', "stdio: 'inherit'", "Network.enable", "Log.enable", 'applicationRootBlank', "Page.reload", 'events.slice(-30)']) {
  assert(principalBrowserRunner.includes(marker), `RMS Principal browser audit is missing required marker ${marker}`);
}
assert(!principalBrowserRunner.includes('vite.real.config.ts'), 'RMS Principal browser audit must not load the Real entry graph');
