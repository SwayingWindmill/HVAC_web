...
const [
  ...
  readText('infra/telemetry/postgres/init/000-bootstrap-identities.sql'),
  readText('infra/telemetry/postgres/init/001-s2-telemetry-baseline.sql'),
  readText('infra/telemetry/postgres/init/002-s2-telemetry-runtime-snapshot.sql'),
  readText('infra/telemetry/postgres/init/004-s2-telemetry-fixtures.sql'),
  readText('modules/telemetry/pkg/telemetryapi/api.gen.go'),
  readText('cmd/energy-api/internal/s2telemetryapi/api.gen.go'),
  readText('apps/hvac-web/src/api/generated/s2Telemetry.gen.ts'),
  readText('go.work'),
  readText('modules/telemetry/go.mod'),
...
assert(compatibility.predecessorContractVersion === '1.0.0', 'predecessor contract version drifted');
assert(compatibility.currentContractVersion === '2.0.0', 'current contract version drifted');
...
assert(ownership.activationStatus === 'v2-active', 'ownership contract activation status drifted');
...
assert(operations.size === compatibility.operations.length, 'public surface operation count differs from the compatibility lock');
...
const goContractFiles = await listSourceFiles('modules/telemetry/pkg/telemetryapi');
assert(exact(goContractFiles, ['modules/telemetry/pkg/telemetryapi/api.gen.go']), 'Telemetry Runtime contract package contains a handwritten DTO file');
...
assert(goWork.includes('./modules/telemetry'), 'Telemetry Runtime module is missing from go.work');
assert(goMod.includes('module github.com/quanlaihe/hvac-web/modules/telemetry'), 'Telemetry Runtime module path drifted');
...
for (const marker of [
  ...
  '018f2d00-0000-7000-8000-000000000002',
...
const queryOperations = new Set(['queryDeviceHistory', 'queryDeviceHistoryAggregate']);
for (const expected of compatibility.operations) {
  ...
  assert(route?.compatibilityMode === 'native' && route?.rollout?.mode === 'all', `${expected.operationId} must remain on the active native route`);
  const expectedOwner = queryOperations.has(expected.operationId) ? 'telemetry-query-service' : 'telemetry-runtime-service';
  assert(route?.owner === expectedOwner, `${expected.operationId} business owner drifted`);
}
...
const expectedScripts = {
  ...
  's2:baseline:check': 'node --experimental-strip-types scripts/check-s2-telemetry-baseline.ts',
...
console.log('S2 Ticket 01 baseline passed: generated contracts, expand-only PostgreSQL, isolated identities and active native ownership routes.');
