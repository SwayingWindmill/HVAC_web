...
export interface MVMonthlyRecord {
  ...
  readonly season: 'SUMMER' | 'TRANSITION';
...
const MOCK_MV_DATA: readonly MVMonthlyRecord[] = [
  { month: '2026-05', season: 'TRANSITION', actualKWh: 88500, adjustedBaselineKWh: 122000, savedKWh: 33500, savedCostRmb: 27800, carbonSavedTon: 17.9, cdd: 85, snapbackRisk: 'normal' },
  { month: '2026-06', season: 'TRANSITION', actualKWh: 142000, adjustedBaselineKWh: 215000, savedKWh: 73000, savedCostRmb: 61200, carbonSavedTon: 39.0, cdd: 180, snapbackRisk: 'normal' },
  { month: '2026-07', season: 'SUMMER', actualKWh: 198000, adjustedBaselineKWh: 295000, savedKWh: 97000, savedCostRmb: 82500, carbonSavedTon: 51.8, cdd: 260, snapbackRisk: 'normal' },
  { month: '2026-08', season: 'SUMMER', actualKWh: 215000, adjustedBaselineKWh: 318000, savedKWh: 103000, savedCostRmb: 88100, carbonSavedTon: 55.0, cdd: 285, snapbackRisk: 'normal' },
  { month: '2026-09 (至今)', season: 'TRANSITION', actualKWh: 154000, adjustedBaselineKWh: 231700, savedKWh: 77700, savedCostRmb: 61400, carbonSavedTon: 41.5, cdd: 195, snapbackRisk: 'normal' },
];

const MV_SEASON_OPTIONS = [
  { label: '盛夏高峰期 (7-8月)', value: 'SUMMER' },
  { label: '过渡季节', value: 'TRANSITION' },
] as const;
const MV_FILTER_COLUMN_IDS = ['season'] as const;
const MV_FILTERS_QUERY_KEY = 'mvFilters';
const MV_JOIN_OPERATOR_QUERY_KEY = 'mvJoinOperator';

function matchesMvAdvancedFilter(row: MVMonthlyRecord, filter: ExtendedColumnFilter<MVMonthlyRecord>) {
  if (filter.id !== 'season') return true;
  const values = Array.isArray(filter.value) ? filter.value : [filter.value];
  switch (filter.operator) {
    case 'eq':
    case 'inArray':
      return values.includes(row.season);
    case 'ne':
    case 'notInArray':
      return !values.includes(row.season);
    case 'isEmpty':
...
export function MeasurementVerificationWorkspace({ siteId: _siteId }: { readonly siteId?: string }) {
  ...
  const [advancedFilters] = useQueryState(
    MV_FILTERS_QUERY_KEY,
    getFiltersStateParser<MVMonthlyRecord>([...MV_FILTER_COLUMN_IDS]).withDefault([]),
  );
  const [joinOperator] = useQueryState(
    MV_JOIN_OPERATOR_QUERY_KEY,
    parseAsStringEnum<JoinOperator>(['and', 'or']).withDefault('and'),
  );

  const filteredData = useMemo(() => MOCK_MV_DATA.filter((row) => {
    if (advancedFilters.length === 0) return true;
    const matches = advancedFilters.map((filter) => matchesMvAdvancedFilter(row, filter));
  ...
  const columns = useMemo<Array<ColumnDef<DataTableFeatures, MVMonthlyRecord>>>(() => [
    ...
    { id: 'month', accessorFn: (row) => row.month, meta: { label: '核算月份' }, header: '核算月份', cell: ({ row }) => <span className="font-mono font-medium text-foreground">{row.original.month}</span> },
    { id: 'season', accessorFn: (row) => row.season, enableColumnFilter: true, meta: { label: '季节阶段', variant: 'select', options: [...MV_SEASON_OPTIONS] }, header: '季节阶段', cell: ({ row }) => <span className="text-xs text-muted-foreground">{row.original.season === 'SUMMER' ? '盛夏高峰期' : '过渡季节'}</span> },
    { id: 'cdd', accessorFn: (row) => row.cdd, meta: { label: '自变量 CDD' }, header: '自变量 CDD', cell: ({ row }) => <span className="font-mono text-muted-foreground tabular-nums">{row.original.cdd} °C·d</span> },
    { id: 'baseline', accessorFn: (row) => row.adjustedBaselineKWh, meta: { label: '天气校正基准能耗' }, header: '天气校正基准能耗', cell: ({ row }) => <span className="font-mono text-muted-foreground tabular-nums">{row.original.adjustedBaselineKWh.toLocaleString()} kWh</span> },
    { id: 'actual', accessorFn: (row) => row.actualKWh, meta: { label: '实测能耗' }, header: '实测能耗', cell: ({ row }) => <span className="font-mono font-semibold text-foreground tabular-nums">{row.original.actualKWh.toLocaleString()} kWh</span> },
    { id: 'savedKWh', accessorFn: (row) => row.savedKWh, meta: { label: '核验净节电量' }, header: '核验净节电量', cell: ({ row }) => <span className="font-mono font-semibold text-emerald-600 tabular-nums">+{row.original.savedKWh.toLocaleString()} kWh</span> },
    { id: 'savedCost', accessorFn: (row) => row.savedCostRmb, meta: { label: '核验节电费' }, header: '核验节电费 (元)', cell: ({ row }) => <span className="font-mono text-foreground tabular-nums">¥{row.original.savedCostRmb.toLocaleString()}</span> },
    { id: 'carbon', accessorFn: (row) => row.carbonSavedTon, meta: { label: '减碳效益' }, header: '减碳效益 (tCO₂e)', cell: ({ row }) => <span className="font-mono text-muted-foreground tabular-nums">{row.original.carbonSavedTon.toFixed(1)} t</span> },
    {
      id: 'snapback',
      accessorFn: (row) => row.snapbackRisk,
      meta: { label: '节能持久性' },
      header: '节能持久性',
      cell: ({ row }) => (
        <StatusBadge
    ...
    <Main className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline" className="text-xs font-normal">IPMVP Option C</Badge>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">已核验节电量</CardTitle>
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">已节省电费</CardTitle>
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">基线模型</CardTitle>
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">节能持久性</CardTitle>
      ...
      <Card className="shadow-xs">
        <CardHeader className="pb-2">
          <CardTitle className="text-base font-semibold">基准与实测用电</CardTitle>
          <CardDescription>
            天气校正后
          </CardDescription>
        </CardHeader>
