...
const SAFETY_CATEGORY_OPTIONS = [...new Set(SAFETY_CHECKS.map((item) => item.category))]
  .map((value) => ({ label: value, value }));
const SAFETY_STATUS_OPTIONS = [
  { label: '检查通过', value: 'PASSED' },
  { label: '边界警示', value: 'WARNING' },
  { label: '禁止下发', value: 'FAILED' },
] as const;
const SAFETY_FILTER_COLUMN_IDS = ['category', 'status'] as const;
const SAFETY_FILTERS_QUERY_KEY = 'optimizationSafetyFilters';
const SAFETY_JOIN_OPERATOR_QUERY_KEY = 'optimizationSafetyJoinOperator';

function matchesSafetyAdvancedFilter(
  item: SafetyCheckItem,
  filter: ExtendedColumnFilter<SafetyCheckItem>,
) {
  const values = Array.isArray(filter.value) ? filter.value : [filter.value];
  const actual = filter.id === 'category' ? item.category : filter.id === 'status' ? item.status : undefined;
...
export function OptimizationPlansConsole({ site }: OptimizationPlansProps) {
  ...
  const [advancedFilters] = useQueryState(
    SAFETY_FILTERS_QUERY_KEY,
    getFiltersStateParser<SafetyCheckItem>([...SAFETY_FILTER_COLUMN_IDS]).withDefault([]),
  );
  const [joinOperator] = useQueryState(
    SAFETY_JOIN_OPERATOR_QUERY_KEY,
    parseAsStringEnum<JoinOperator>(['and', 'or']).withDefault('and'),
  );
  const filteredChecks = useMemo(() => {
    return SAFETY_CHECKS.filter((item) => {
      const q = search.trim().toLowerCase();
      const matchesSearch = !q || item.name.toLowerCase().includes(q) || item.category.toLowerCase().includes(q) || item.basis.toLowerCase().includes(q);
      const filterMatches = advancedFilters.map((filter) => matchesSafetyAdvancedFilter(item, filter));
  ...
  const columns = useMemo<Array<ColumnDef<DataTableFeatures, SafetyCheckItem>>>(() => [
    ...
    { id: 'name', accessorFn: (row) => row.name, meta: { label: '检查维度' }, header: '检查维度', cell: ({ row }) => <span className="font-medium text-foreground">{row.original.name}</span> },
    { id: 'category', accessorFn: (row) => row.category, enableColumnFilter: true, meta: { label: '检查类别', variant: 'select', options: SAFETY_CATEGORY_OPTIONS }, header: '类别', cell: ({ row }) => <span className="text-sm text-muted-foreground">{row.original.category}</span> },
    {
      id: 'status',
      accessorFn: (row) => row.status,
      enableColumnFilter: true,
      meta: { label: '检查状态', variant: 'select', options: [...SAFETY_STATUS_OPTIONS] },
      header: '检查状态',
      cell: ({ row }) => (
        <StatusBadge
  ...
  const table = useDataTable({
    ...
    meta: {
      queryKeys: {
        page: 'optimizationSafetyPage',
        perPage: 'optimizationSafetyPerPage',
        sort: 'optimizationSafetySort',
        filters: SAFETY_FILTERS_QUERY_KEY,
        joinOperator: SAFETY_JOIN_OPERATOR_QUERY_KEY,
      },
    },
  });
    ...
    <Main className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold tracking-tight text-foreground">优化方案</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">推荐方案</CardTitle>
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">预计 COP</CardTitle>
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">预计节电</CardTitle>
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">执行前检查</CardTitle>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="shadow-xs lg:col-span-2">
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-lg">运行对比</CardTitle>
        ...
        <Card className="shadow-xs flex flex-col justify-between">
          <CardHeader>
            <CardTitle className="text-base">审批</CardTitle>
      ...
      >
        <DataTable
          ...
          footer={<DataTablePagination table={table} totalRows={filteredChecks.length} />}
        >
      ...
      <Dialog open={approveDialogOpen} onOpenChange={setApproveDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              确认将「冷冻水供水温度自适应提升 (7.0°C → 8.0°C)」下发至控制？
            </DialogDescription>
          </DialogHeader>
