...
const PORTFOLIO_REGION_OPTIONS = [...new Set(PORTFOLIO_SITES.map((site) => site.region))]
  .map((value) => ({ label: value, value }));
const PORTFOLIO_STATUS_OPTIONS = [
  { label: '正常运行', value: 'normal' },
  { label: '能效偏离', value: 'attention' },
  { label: '关键风险', value: 'critical' },
] as const;
const PORTFOLIO_FILTER_COLUMN_IDS = ['region', 'status'] as const;
const PORTFOLIO_FILTERS_QUERY_KEY = 'portfolioSiteFilters';
const PORTFOLIO_JOIN_OPERATOR_QUERY_KEY = 'portfolioSiteJoinOperator';

function matchesPortfolioAdvancedFilter(
  site: PortfolioSite,
  filter: ExtendedColumnFilter<PortfolioSite>,
) {
  const values = Array.isArray(filter.value) ? filter.value : [filter.value];
  const actual = filter.id === 'region' ? site.region : filter.id === 'status' ? site.status : undefined;
...
export function PortfolioOverviewWorkspace() {
  ...
  const [siteAdvancedFilters] = useQueryState(
    PORTFOLIO_FILTERS_QUERY_KEY,
    getFiltersStateParser<PortfolioSite>([...PORTFOLIO_FILTER_COLUMN_IDS]).withDefault([]),
  );
  const [siteJoinOperator] = useQueryState(
    PORTFOLIO_JOIN_OPERATOR_QUERY_KEY,
  ...
  const filteredSites = useMemo(() => {
    return PORTFOLIO_SITES.filter((site) => {
      ...
      const filterMatches = siteAdvancedFilters.map((filter) => matchesPortfolioAdvancedFilter(site, filter));
      const matchAdvancedFilters = siteAdvancedFilters.length === 0 || (siteJoinOperator === 'or'
  ...
  const siteColumns = useMemo<Array<ColumnDef<DataTableFeatures, PortfolioSite>>>(() => [
    ...
    { id: 'region', accessorFn: (row) => row.region, enableColumnFilter: true, meta: { label: '大区', variant: 'select', options: PORTFOLIO_REGION_OPTIONS }, header: '大区 / 面积', cell: ({ row }) => <div><div className="font-medium text-foreground">{row.original.region}</div><div className="font-mono text-[11px] text-muted-foreground tabular-nums">{(row.original.buildingArea / 10000).toFixed(1)} 万 m²</div></div> },
    { id: 'hvacType', accessorFn: (row) => row.hvacType, meta: { label: '冷站系统架构' }, header: '冷站系统架构', cell: ({ row }) => <span className="text-muted-foreground">{row.original.hvacType}</span> },
    {
      id: 'cop',
      accessorFn: (row) => row.currentCop,
      meta: { label: '当前 COP' },
      header: '当前 COP / 设计',
    ...
    { id: 'consumption', accessorFn: (row) => row.monthlyConsumption, meta: { label: '月度用能' }, header: '月度用能 (MWh)', cell: ({ row }) => <span className="font-mono font-medium text-foreground tabular-nums">{row.original.monthlyConsumption.toFixed(1)}</span> },
    { id: 'eui', accessorFn: (row) => row.eui, meta: { label: '实测 EUI' }, header: '实测 EUI / 气象归一化', cell: ({ row }) => <span className="font-mono tabular-nums"><span className="font-medium text-foreground">{row.original.eui.toFixed(1)}</span><span className="text-[11px] text-muted-foreground"> / {row.original.normalizedEui.toFixed(1)}</span></span> },
    { id: 'peerRank', accessorFn: (row) => row.peerRank, meta: { label: '同行排位' }, header: '同行排位', cell: ({ row }) => <span className="font-medium text-muted-foreground">{row.original.peerRank}</span> },
    {
      id: 'completeness',
      accessorFn: (row) => row.dataCompleteness,
      meta: { label: '采集完整率' },
      header: '采集完整率',
    ...
    },
    {
      ...
      meta: { label: '运行状态', variant: 'select', options: [...PORTFOLIO_STATUS_OPTIONS] },
      header: '运行状态',
      cell: ({ row }) => row.original.status === 'critical'
        ? <StatusBadge label={`关键风险 (${row.original.highRiskAlerts})`} tone="destructive" pulse />
        : row.original.status === 'attention'
          ? <StatusBadge label={`能效偏离 (${row.original.activeAlerts})`} tone="warning" />
          : <StatusBadge label="工况达标" tone="success" />,
    },
    {
      id: 'action',
      header: '操作',
  ...
  const siteTable = useDataTable({
    ...
    meta: {
      queryKeys: {
        ...
        perPage: 'portfolioSitePerPage',
        sort: 'portfolioSiteSort',
        filters: PORTFOLIO_FILTERS_QUERY_KEY,
        joinOperator: PORTFOLIO_JOIN_OPERATOR_QUERY_KEY,
      },
    },
  });
  ...
  const opportunityColumns = useMemo<Array<ColumnDef<DataTableFeatures, PortfolioOpportunity>>>(() => [
    ...
    { id: 'status', header: '当前状态', cell: ({ row }) => <StatusBadge label={row.original.status} tone={row.original.status === '执行中' ? 'success' : row.original.status === '待立项' ? 'warning' : 'info'} /> },
    {
      id: 'actions',
      header: '操作',
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">本月总用电</CardTitle>
