...
const METER_LEVEL_OPTIONS = [
  { label: '总进线', value: 'MAIN' },
  { label: '一级支路', value: 'BRANCH' },
  { label: '虚拟差额', value: 'VIRTUAL' },
] as const;
const METER_FILTER_COLUMN_IDS = ['level'] as const;
const METER_FILTERS_QUERY_KEY = 'meterModelFilters';
const METER_JOIN_OPERATOR_QUERY_KEY = 'meterModelJoinOperator';

function matchesMeterAdvancedFilter(meter: MeterNode, filter: ExtendedColumnFilter<MeterNode>) {
  if (filter.id !== 'level') return true;
  const values = Array.isArray(filter.value) ? filter.value : [filter.value];
  switch (filter.operator) {
    case 'eq':
    case 'inArray':
      return values.includes(meter.level);
    case 'ne':
    case 'notInArray':
      return !values.includes(meter.level);
    case 'isEmpty':
...
const SEMANTIC_UNIT_OPTIONS = [...new Set(SEMANTIC_POINTS.map((point) => point.unit))]
  .map((value) => ({ label: value, value }));
const SEMANTIC_DEVICE_OPTIONS = [...new Set(SEMANTIC_POINTS.map((point) => point.device))]
  .map((value) => ({ label: value, value }));
const SEMANTIC_POINT_FILTER_COLUMN_IDS = ['device', 'unit'] as const;
const SEMANTIC_POINT_FILTERS_QUERY_KEY = 'semanticPointFilters';
const SEMANTIC_POINT_JOIN_OPERATOR_QUERY_KEY = 'semanticPointJoinOperator';

function matchesSemanticPointAdvancedFilter(
  point: SemanticPoint,
  filter: ExtendedColumnFilter<SemanticPoint>,
) {
  if (filter.id !== 'unit' && filter.id !== 'device') return true;
  const values = Array.isArray(filter.value) ? filter.value : [filter.value];
  const actual = filter.id === 'device' ? point.device : point.unit;
  switch (filter.operator) {
    case 'eq':
    case 'inArray':
...
export function MeteringSemanticModelWorkspace({ siteId: _siteId }: MeteringSemanticModelWorkspaceProps) {
  ...
  const [meterAdvancedFilters] = useQueryState(
    METER_FILTERS_QUERY_KEY,
    getFiltersStateParser<MeterNode>([...METER_FILTER_COLUMN_IDS]).withDefault([]),
  );
  const [meterJoinOperator] = useQueryState(
    METER_JOIN_OPERATOR_QUERY_KEY,
    parseAsStringEnum<JoinOperator>(['and', 'or']).withDefault('and'),
  );

  const filteredMeters = useMemo(() => {
    return METER_HIERARCHY.filter((meter) => {
      const q = meterSearch.trim().toLowerCase();
      const matchesSearch = !q || meter.name.toLowerCase().includes(q) || meter.code.toLowerCase().includes(q) || meter.type.toLowerCase().includes(q);
      const filterMatches = meterAdvancedFilters.map((filter) => matchesMeterAdvancedFilter(meter, filter));
      const matchesAdvancedFilters = meterAdvancedFilters.length === 0 || (meterJoinOperator === 'or'
        ? filterMatches.some(Boolean)
        : filterMatches.every(Boolean));
      return matchesSearch && matchesAdvancedFilters;
    });
  }, [meterAdvancedFilters, meterJoinOperator, meterSearch]);

  const [pointSearch, setPointSearch] = useState('');
  const [pointAdvancedFilters] = useQueryState(
    SEMANTIC_POINT_FILTERS_QUERY_KEY,
    getFiltersStateParser<SemanticPoint>([...SEMANTIC_POINT_FILTER_COLUMN_IDS]).withDefault([]),
  );
  const [pointJoinOperator] = useQueryState(
    SEMANTIC_POINT_JOIN_OPERATOR_QUERY_KEY,
    parseAsStringEnum<JoinOperator>(['and', 'or']).withDefault('and'),
  );

  const filteredPoints = useMemo(() => {
    return SEMANTIC_POINTS.filter((point) => {
      const q = pointSearch.trim().toLowerCase();
      const matchesSearch = !q || (
        point.pointTag.toLowerCase().includes(q) ||
        point.name.toLowerCase().includes(q) ||
        point.brickClass.toLowerCase().includes(q) ||
        point.device.toLowerCase().includes(q)
      );
      const filterMatches = pointAdvancedFilters.map((filter) => matchesSemanticPointAdvancedFilter(point, filter));
      const matchesAdvancedFilters = pointAdvancedFilters.length === 0 || (pointJoinOperator === 'or'
        ? filterMatches.some(Boolean)
        : filterMatches.every(Boolean));
      return matchesSearch && matchesAdvancedFilters;
    });
  }, [pointAdvancedFilters, pointJoinOperator, pointSearch]);
  ...
  const meterColumns = useMemo<Array<ColumnDef<DataTableFeatures, MeterNode>>>(() => [
    ...
    { id: 'code', accessorFn: (row) => row.code, meta: { label: '表具编码' }, header: '表具编码', cell: ({ row }) => <span className="font-mono text-[11px] font-medium text-foreground">{row.original.code}</span> },
    {
      id: 'name',
    ...
    },
    {
      ...
      meta: { label: '拓扑层级', variant: 'select', options: [...METER_LEVEL_OPTIONS] },
      header: '拓扑层级',
      cell: ({ row }) => <StatusBadge label={row.original.level === 'MAIN' ? '总进线' : row.original.level === 'VIRTUAL' ? '虚拟差额' : '一级支路'} tone={row.original.level === 'MAIN' ? 'info' : row.original.level === 'VIRTUAL' ? 'warning' : 'neutral'} />,
    },
    ...
    { id: 'readingCurrentKw', accessorFn: (row) => row.readingCurrentKw, meta: { label: '当前实时负荷' }, header: '当前实时负荷', cell: ({ row }) => <span className="font-mono text-[11px] font-medium text-foreground tabular-nums">{row.original.readingCurrentKw.toLocaleString()} kW</span> },
    { id: 'shareOfMainPercent', accessorFn: (row) => row.shareOfMainPercent, meta: { label: '进线占比' }, header: '进线占比', cell: ({ row }) => <span className="font-mono text-[11px] text-muted-foreground tabular-nums">{row.original.shareOfMainPercent}%</span> },
    { id: 'dailyTotalKwh', accessorFn: (row) => row.dailyTotalKwh, meta: { label: '当日累计电量' }, header: '当日累计电量', cell: ({ row }) => <span className="font-mono text-[11px] text-muted-foreground tabular-nums">{row.original.dailyTotalKwh.toLocaleString()} kWh</span> },
    {
      id: 'balanceDiff',
      header: '平账状态 / 差额比',
      cell: ({ row }) => <StatusBadge label={row.original.balanceDiff} tone={row.original.balanceDiff.includes('达标') || row.original.balanceDiff.includes('正常') ? 'success' : 'warning'} />,
    },
  ], []);
  ...
  const meterTable = useDataTable({
    ...
    meta: {
      queryKeys: {
        page: 'meterModelPage',
        perPage: 'meterModelPerPage',
        sort: 'meterModelSort',
        filters: METER_FILTERS_QUERY_KEY,
        joinOperator: METER_JOIN_OPERATOR_QUERY_KEY,
      },
    },
  });
  ...
  const pointColumns = useMemo<Array<ColumnDef<DataTableFeatures, SemanticPoint>>>(() => [
    ...
    { id: 'pointTag', accessorFn: (row) => row.pointTag, meta: { label: '点位标签' }, header: '点位标签 (Tag)', cell: ({ row }) => <span className="font-mono text-[11px] font-medium text-foreground">{row.original.pointTag}</span> },
    { id: 'name', accessorFn: (row) => row.name, meta: { label: '物理测点语义描述' }, header: '物理测点语义描述', cell: ({ row }) => <span className="font-medium leading-snug text-foreground">{row.original.name}</span> },
    { id: 'brickClass', accessorFn: (row) => row.brickClass, meta: { label: 'Brick Schema 类' }, header: 'Brick Schema 类', cell: ({ row }) => <span className="font-mono text-[11px] text-primary">{row.original.brickClass}</span> },
    ...
    { id: 'device', accessorFn: (row) => row.device, enableColumnFilter: true, meta: { label: '所属物理设备', variant: 'select', options: SEMANTIC_DEVICE_OPTIONS }, header: '所属物理设备', cell: ({ row }) => <StatusBadge label={row.original.device} tone="neutral" /> },
    { id: 'unit', accessorFn: (row) => row.unit, enableColumnFilter: true, meta: { label: '工程单位', variant: 'select', options: SEMANTIC_UNIT_OPTIONS }, header: '工程单位', cell: ({ row }) => <span className="font-mono font-medium text-foreground">{row.original.unit}</span> },
  ...
  const pointTable = useDataTable({
    ...
    meta: {
      queryKeys: {
        page: 'semanticPointPage',
        perPage: 'semanticPointPerPage',
        sort: 'semanticPointSort',
        filters: SEMANTIC_POINT_FILTERS_QUERY_KEY,
        joinOperator: SEMANTIC_POINT_JOIN_OPERATOR_QUERY_KEY,
      },
    },
  });
    ...
    <Main className="space-y-6">
      ...
      <Tabs defaultValue="meters" className="space-y-4">
        ...
        <TabsContent value="meters" className="space-y-4 mt-0">
          ...
          <DataTableBlock aria-label="计量回路拓扑树">
            <DataTable
              table={meterTable}
            ...
            getRowProps={(row) => ({
              className: row.original.level === 'MAIN' ? 'bg-muted/10 font-semibold' : undefined,
            })}
            getCellProps={(cell) => ({
              className:
          ...
          >
            <DataTableAdvancedToolbar table={meterTable}>
              ...
              <DataTableSortList table={meterTable} />
              <DataTableFilterList table={meterTable} />
        ...
        <TabsContent value="ontology" className="space-y-3 mt-0">
          <DataTableBlock aria-label="标准化语义点位字典">
            <DataTable
              table={pointTable}
          ...
          >
            <DataTableAdvancedToolbar table={pointTable}>
              <div className="relative w-64">
                <Search className="absolute left-2.5 top-2.5 h-3.5 w-3.5 text-muted-foreground" />
                <Input
              ...
              <DataTableSortList table={pointTable} />
              <DataTableFilterList table={pointTable} />
              <div className="text-xs text-muted-foreground">
