  ...
  return (
    <Main fluid className="space-y-4" data-testid="alarm-center" data-site-id={site.id} data-view={view}>
          ...
          >
            ...
            {listQuery.isError && alarms.length === 0 ? (
              ...
              <DataTable
                ...
                tableAriaLabel="告警"
                empty={view === 'active' ? '当前筛选条件下没有活动告警' : view === 'suppressed' ? '当前没有已搁置告警' : '当前筛选条件下没有历史告警'}
                getHeaderRowProps={() => ({
              ...
              >
                <div className="flex flex-wrap items-center gap-2" data-testid="alarm-triage-toolbar">
                  ...
                  <Select value={severity} onValueChange={(value) => onSearchChange({ severity: value === 'all' ? undefined : value as AlarmSeverity, selected: undefined })}>
                    <SelectTrigger className="h-8 w-[110px] text-xs"><SelectValue placeholder="全部等级" /></SelectTrigger>
                  ...
                  <Select value={ack} onValueChange={(value) => onSearchChange({ ack: value === 'all' ? undefined : value as Exclude<AlarmAcknowledgementFilter, 'all'>, selected: undefined })}>
                    ...
                    <SelectContent><SelectGroup><SelectItem value="all">全部确认</SelectItem><SelectItem value="unacknowledged">未确认</SelectItem><SelectItem value="acknowledged">已确认</SelectItem></SelectGroup></SelectContent>
                  </Select>
                  <Select value={owner} onValueChange={(value) => onSearchChange({ owner: value === 'all' ? undefined : value as Exclude<AlarmOwnershipFilter, 'all'>, selected: undefined })}>
                    <SelectTrigger className="h-8 w-[110px] text-xs"><SelectValue /></SelectTrigger>
                    <SelectContent><SelectGroup><SelectItem value="all">全部责任</SelectItem><SelectItem value="unassigned">未指派</SelectItem><SelectItem value="assigned">已指派</SelectItem></SelectGroup></SelectContent>
                  </Select>
      ...
      <Sheet open={Boolean(selectedAlarmId) && compactInspector} onOpenChange={(open) => { if (!open) closeInspector(); }}>
        <SheetContent
          className="xl:hidden sm:max-w-[540px]!"
          aria-label="告警详情"
        ...
        >
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-4 pt-4 pb-4">{inspectorBody}</div>
        </SheetContent>
      </Sheet>
