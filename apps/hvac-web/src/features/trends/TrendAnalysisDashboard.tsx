...
interface VariableConfig {
  ...
  readonly channelType: 'temperature' | 'power' | 'calculated';
...
const VARIABLES: readonly VariableConfig[] = [
  { id: 'chw_supply_temp', name: '冷冻水供水温度', unit: '°C', channelType: 'temperature', color: '#2563eb', axis: 'left', min: 6.8, max: 7.6, current: 7.0, avg: 7.1, sensor: 'CH-TT-01 (出水温变送器)', status: 'normal' },
  { id: 'chw_return_temp', name: '冷冻水回水温度', unit: '°C', channelType: 'temperature', color: '#06b6d4', axis: 'left', min: 11.2, max: 13.5, current: 12.4, avg: 12.2, sensor: 'CH-TT-02 (回水温变送器)', status: 'normal' },
  { id: 'cw_return_temp', name: '冷却水出水温度', unit: '°C', channelType: 'temperature', color: '#f59e0b', axis: 'left', min: 28.5, max: 33.8, current: 33.0, avg: 31.5, sensor: 'CT-TT-01 (冷却塔回水温)', status: 'normal' },
  { id: 'plant_power', name: '冷站实时总功率', unit: 'kW', channelType: 'power', color: '#8b5cf6', axis: 'right', min: 180, max: 540, current: 504, avg: 420, sensor: 'PM-MAIN-01 (总电力仪表)', status: 'normal' },
  { id: 'system_cop', name: '系统综合能效 COP', unit: '', channelType: 'calculated', color: '#10b981', axis: 'right', min: 4.5, max: 6.4, current: 5.8, avg: 5.6, sensor: 'CALC-PLANT-COP (群控算法)', status: 'normal' },
];

const TREND_STATUS_OPTIONS = [
  { label: '质量正常', value: 'normal' },
  { label: '质量告警', value: 'warning' },
] as const;
const TREND_CHANNEL_TYPE_OPTIONS = [
  { label: '温度通道', value: 'temperature' },
  { label: '功率通道', value: 'power' },
  { label: '计算指标', value: 'calculated' },
] as const;
const TREND_FILTER_COLUMN_IDS = ['channelType', 'status'] as const;
const TREND_FILTERS_QUERY_KEY = 'trendFilters';
const TREND_JOIN_OPERATOR_QUERY_KEY = 'trendJoinOperator';

function matchesTrendAdvancedFilter(
  variable: VariableConfig,
  filter: ExtendedColumnFilter<VariableConfig>,
) {
  if (filter.id !== 'status' && filter.id !== 'channelType') return true;
  const values = Array.isArray(filter.value) ? filter.value : [filter.value];
  const actual = filter.id === 'status' ? variable.status : variable.channelType;
...
export function TrendAnalysisDashboard({ site }: TrendAnalysisDashboardProps) {
  ...
  const [advancedFilters] = useQueryState(
    TREND_FILTERS_QUERY_KEY,
    getFiltersStateParser<VariableConfig>([...TREND_FILTER_COLUMN_IDS]).withDefault([]),
  );
  const [joinOperator] = useQueryState(
    TREND_JOIN_OPERATOR_QUERY_KEY,
    parseAsStringEnum<JoinOperator>(['and', 'or']).withDefault('and'),
  );
  const filteredVariables = useMemo(() => {
    return VARIABLES.filter((variable) => {
      if (channelSearch && !variable.name.toLowerCase().includes(channelSearch.toLowerCase()) && !variable.sensor.toLowerCase().includes(channelSearch.toLowerCase())) {
        return false;
      }
      const filterMatches = advancedFilters.map((filter) => matchesTrendAdvancedFilter(variable, filter));
      return advancedFilters.length === 0 || (joinOperator === 'or'
        ? filterMatches.some(Boolean)
        : filterMatches.every(Boolean));
    });
  }, [advancedFilters, channelSearch, joinOperator]);
  ...
  const columns = useMemo<Array<ColumnDef<DataTableFeatures, VariableConfig>>>(() => [
    ...
    },
    {
      ...
      meta: { label: '通道类型', variant: 'select', options: [...TREND_CHANNEL_TYPE_OPTIONS] },
      header: '通道类型',
      cell: ({ row }) => <span className="text-xs text-muted-foreground">{row.original.channelType === 'temperature' ? '温度通道' : row.original.channelType === 'power' ? '功率通道' : '计算指标'}</span>,
    },
    { id: 'sensor', accessorFn: (row) => row.sensor, meta: { label: '关联传感器' }, header: '关联传感器与物理点位', cell: ({ row }) => <span className="font-mono text-[11px] text-muted-foreground">{row.original.sensor}</span> },
    { id: 'current', accessorFn: (row) => row.current, meta: { label: '当前采样值' }, header: '当前采样值', cell: ({ row }) => <span className="font-mono font-semibold text-foreground tabular-nums">{row.original.current} {row.original.unit}</span> },
    { id: 'min', accessorFn: (row) => row.min, meta: { label: '区间极小值' }, header: '区间极小值', cell: ({ row }) => <span className="font-mono text-muted-foreground tabular-nums">{row.original.min} {row.original.unit}</span> },
    { id: 'max', accessorFn: (row) => row.max, meta: { label: '区间极大值' }, header: '区间极大值', cell: ({ row }) => <span className="font-mono text-muted-foreground tabular-nums">{row.original.max} {row.original.unit}</span> },
    { id: 'avg', accessorFn: (row) => row.avg, meta: { label: '区间均值' }, header: '区间均值', cell: ({ row }) => <span className="font-mono text-muted-foreground tabular-nums">{row.original.avg} {row.original.unit}</span> },
    { id: 'status', accessorFn: (row) => row.status, enableColumnFilter: true, meta: { label: '数据质量', variant: 'select', options: [...TREND_STATUS_OPTIONS] }, header: '数据质量', cell: ({ row }) => <StatusBadge label={row.original.status === 'normal' ? '完整新鲜' : '质量告警'} tone={row.original.status === 'normal' ? 'success' : 'warning'} /> },
  ], []);
  ...
  const table = useDataTable({
    ...
    meta: {
      queryKeys: {
        page: 'trendPage',
        perPage: 'trendPerPage',
        sort: 'trendSort',
        filters: TREND_FILTERS_QUERY_KEY,
        joinOperator: TREND_JOIN_OPERATOR_QUERY_KEY,
      },
    },
  });
    ...
    <Main className="space-y-6" data-testid="trend-analysis-dashboard">
      ...
      <Card className="shadow-xs">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <div>
            <CardTitle className="text-base font-semibold">测点趋势</CardTitle>
            <CardDescription className="text-xs">温度、功率与 COP</CardDescription>
          </div>
