...
export function CarbonEmissionsDashboard({ siteId: _siteId }: { readonly siteId?: string }) {
  ...
  const factorColumns = useMemo<Array<ColumnDef<DataTableFeatures, EmissionFactorRow>>>(() => [
    ...
    { id: 'validFrom', header: '状态/生效日期', cell: ({ row }) => <StatusBadge tone={row.original.factor === '0.0000' ? 'success' : 'neutral'} label={row.original.validFrom} /> },
  ], []);
  ...
  const reductionColumns = useMemo<Array<ColumnDef<DataTableFeatures, ReductionProjectRow>>>(() => [
    ...
    { id: 'carbonSaved', header: '减碳贡献', cell: ({ row }) => <span className="font-mono font-semibold text-foreground tabular-nums">{row.original.carbonSaved}</span> },
    {
      ...
      cell: ({ row }) => {
        ...
        return <StatusBadge tone={isVerified ? 'success' : isRunning ? 'info' : 'in-progress'} pulse={!isVerified && !isRunning} label={row.original.status} />;
      },
    },
  ], []);
  ...
  return (
    <Main className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">本年净排放</CardTitle>
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">碳排放强度</CardTitle>
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">碳配额使用率</CardTitle>
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">绿电占比</CardTitle>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <DataTableBlock
          title="排放因子依据"
          actions={<StatusBadge tone="info" label="2024基准" />}
        >
          <DataTable
            table={factorTable}
        ...
        <DataTableBlock
          title="节能减碳项目贡献"
          actions={<StatusBadge tone="success" label="3 个实施项目" />}
        >
          <DataTable
