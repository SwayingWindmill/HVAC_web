      ...
      <CardContent className="p-0">
        <dl className="divide-y text-xs">
          <div className="flex items-center justify-between px-4 py-2.5">
            <dt className="flex items-center gap-1.5 text-muted-foreground"><Building2 className="size-3.5 text-muted-foreground" />站点</dt>
            <dd className="font-semibold text-foreground">{site.displayName}</dd>
          </div>
          <div className="flex items-center justify-between px-4 py-2.5">
            <dt className="flex items-center gap-1.5 text-muted-foreground"><MapPin className="size-3.5 text-muted-foreground" />空间位置</dt>
            <dd className="font-semibold text-foreground">{deviceLocation(row)}</dd>
          </div>
          <div className="flex items-center justify-between px-4 py-2.5">
            <dt className="flex items-center gap-1.5 text-muted-foreground"><Server className="size-3.5 text-muted-foreground" />关联资产</dt>
            <dd className="font-semibold text-foreground">{assetNames.length ? assetNames.join('、') : '未关联'}</dd>
          </div>
          <div className="flex items-center justify-between px-4 py-2.5">
            <dt className="flex items-center gap-1.5 text-muted-foreground"><Radio className="size-3.5 text-muted-foreground" />物理传感器</dt>
            <dd className="font-semibold tabular-nums text-foreground">{sensorCount} 个</dd>
          </div>
          <div className="flex items-center justify-between px-4 py-2.5">
            <dt className="flex items-center gap-1.5 text-muted-foreground"><Boxes className="size-3.5 text-muted-foreground" />注册点位</dt>
...
function EngineeringPoints({ row }: { readonly row: AssetsDeviceRow }) {
  ...
  return (
    <DataTableBlock
      title={<span className="flex items-center gap-2"><Cpu className="size-4.5 text-primary" aria-hidden="true" />工程点位</span>}
      description="设备已登记点位及当前观测。源端“可写”仅表示对象能力，不代表当前用户可执行控制。"
    >
      <DataTable
        table={table}
        role="region"
      ...
      <CardContent className="p-0">
        <dl className="divide-y text-xs">
          <div className="flex items-center justify-between px-4 py-2.5">
            ...
            <dd className="rounded bg-muted/40 px-1.5 py-0.5 font-mono font-medium text-foreground">{row.device.code}</dd>
          </div>
          <div className="flex items-center justify-between px-4 py-2.5">
          ...
          <div className="flex items-center justify-between px-4 py-2.5">
            <dt className="flex items-center gap-1 text-muted-foreground"><Calendar className="size-3" />登记时间</dt>
            <dd className="font-medium tabular-nums text-foreground">{formatTimestamp(row.device.createdAt, timezone)}</dd>
          </div>
          <div className="flex items-center justify-between px-4 py-2.5">
            <dt className="flex items-center gap-1 text-muted-foreground"><Clock className="size-3" />资料更新</dt>
  ...
  return (
    <Card size="sm" className="shadow-xs">
      ...
      <CardContent className="space-y-1 p-2">
        <Button variant="ghost" size="sm" className="h-8 w-full justify-between text-xs font-normal" asChild>
        ...
        <Button variant="ghost" size="sm" className="h-8 w-full justify-between text-xs font-normal" asChild>
          <Link to="/sites/$siteId/alarms" params={{ siteId: site.id }} search={{ view: 'active', source: 'device-detail', device: row.device.displayName, deviceId: row.device.id }}>
            <span className="flex items-center gap-2"><AlertTriangle className="size-3.5 text-muted-foreground" />告警</span>
            <ArrowRight className="size-3 text-muted-foreground" />
          </Link>
        </Button>
        <Button variant="ghost" size="sm" className="h-8 w-full justify-between text-xs font-normal" asChild>
        ...
        <Button variant="ghost" size="sm" className="h-8 w-full justify-between text-xs font-normal" asChild>
          <Link to="/sites/$siteId/control" params={{ siteId: site.id }}>
            <span className="flex items-center gap-2"><Cpu className="size-3.5 text-muted-foreground" />控制</span>
            <ArrowRight className="size-3 text-muted-foreground" />
          </Link>
        </Button>
      </CardContent>
...
export function AssetDeviceDetail({ site, principal, runtime, deviceId, onBack }: AssetDeviceDetailProps) {
  ...
  const stateItems = [
    { label: '运行状态', value: running.label, icon: <Activity />, tone: running.label === '运行中' || running.label === '已停止' ? 'default' : 'warning' },
    { label: '连接状态', value: connection.label, icon: <Radio />, tone: connection.label === '在线' || connection.label === '不适用' ? 'default' : connection.label === '离线' ? 'critical' : 'warning' },
    { label: '数据新鲜度', value: freshnessLabel(row.operational.telemetry.freshness), detail: latestTimestamp(row, site.timezone), icon: <Clock />, tone: row.operational.telemetry.freshness === 'FRESH' ? 'default' : 'warning' },
    { label: '数据质量', value: qualityLabel(row.operational.telemetry.quality), icon: <CheckCircle2 />, tone: row.operational.telemetry.quality === 'GOOD' ? 'default' : 'warning' },
  ] as const;
  ...
  return (
    <Main fluid className="space-y-5" data-testid="asset-device-detail" data-device-visible="true">
      ...
      <section className="space-y-3" aria-label="设备身份与当前状态">
        ...
        <FactStrip items={stateItems} ariaLabel="设备独立状态" />
      </section>
        ...
        <div className="min-w-0 space-y-4">
          <Card className="shadow-xs">
            <CardHeader className="border-b bg-muted/10 pb-3">
              ...
              <CardDescription className="text-xs text-muted-foreground">显示采样时间与质量状态</CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              {data.currentPending ? (
                ...
                <div className="grid divide-y sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-3">
                  {points.slice(0, 8).map((point) => {
                    ...
                    return (
                      <div key={point.pointId} className="min-w-0 space-y-1.5 p-4 transition-colors hover:bg-muted/15">
                        <div className="flex items-center justify-between">
                          <span className="block truncate text-xs font-medium text-muted-foreground">{point.label}</span>
                        ...
                        <span className={cn('block truncate text-[11px]', point.quality === 'GOOD' && point.freshness === 'FRESH' ? 'text-muted-foreground' : 'font-medium text-warning')}>
          ...
          <Card size="sm" className="shadow-xs" aria-label="当前注意事项">
            <CardHeader className="border-b bg-muted/10 pb-3">
              ...
              <CardDescription className="text-xs text-muted-foreground">连接异常和数据质量问题</CardDescription>
            </CardHeader>
            <CardContent className="pt-3.5">
              {row.operational.needsAttention ? (
                <div className="flex flex-wrap gap-2">
                  {row.operational.attentionReasons.map((reason) => (
                    <Badge key={reason} variant="outline" className="border-warning/35 bg-warning/10 text-xs text-warning">
