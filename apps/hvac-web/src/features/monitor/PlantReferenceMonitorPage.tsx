...
export function PlantReferenceMonitorPage({
  ...
  const getRunStatePill = (points: Parameters<typeof monitorRunState>[0]) => {
    ...
    return <StatusBadge tone={tone} label={monitorRunStateLabel(rs)} />;
  };
    ...
    <div className="hvac-plant-ref" data-testid="hvac-plant-reference-page">
      ...
      <section className="hvac-plant-ref__main-grid">
        ...
        <div className="hvac-plant-ref__right-stack">
          <PlantPanel title="冷机群运行概况" className="hvac-plant-ref__chiller-overview">
            <Table aria-label="冷机群运行概况">
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                    <TableHead>冷机编号</TableHead>
                    <TableHead className="w-28 text-center">运行状态</TableHead>
                    <TableHead className="text-right">负荷率</TableHead>
                    <TableHead className="text-right">冷量 (kW)</TableHead>
                    <TableHead className="text-right">输入功率 (kW)</TableHead>
                    <TableHead className="text-right">COP</TableHead>
                    <TableHead className="text-right">冷冻水进/回水 (°C)</TableHead>
                    <TableHead className="text-right">冷却水进/出水 (°C)</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {chillers.slice(0, 3).map((item) => {
                    const state = monitorRunState(item.state.points);
                    return (
                      <TableRow
                        key={item.device.id}
                        className="cursor-pointer"
                        onClick={(event) => {
                          if (!(event.target as HTMLElement).closest('button,a')) onOpenDevice(item.device.id);
                        }}
                      >
                        <TableCell><Button variant="link" className="h-auto p-0 font-medium" onClick={() => onOpenDevice(item.device.id)}>{shortEquipmentName(item.device.displayName)}</Button></TableCell>
                        <TableCell className="text-center"><StatusBadge tone={state === 'RUNNING' ? 'success' : state === 'FAULT' ? 'destructive' : 'neutral'} label={monitorRunStateLabel(state)} /></TableCell>
                        <TableCell className="text-right font-mono tabular-nums">{pointText(item, ['load_rate', '负荷率'], 0)}</TableCell>
                        <TableCell className="text-right font-mono tabular-nums">{pointText(item, ['chiller.cooling_capacity', '制冷量'], 0)}</TableCell>
                        <TableCell className="text-right font-mono tabular-nums">{pointText(item, ['chiller.power', '功率'], 0)}</TableCell>
                        <TableCell className="text-right font-mono tabular-nums">{pointText(item, ['chiller.cop', 'cop'], 1)}</TableCell>
                        <TableCell className="text-right font-mono tabular-nums">{`${pointText(item, ['chilled_water_supply_temperature', '冷冻水供水'], 1).replace(/ °C$/, '')} / ${pointText(item, ['chilled_water_return_temperature', '冷冻水回水'], 1).replace(/ °C$/, '')}`}</TableCell>
                        <TableCell className="text-right font-mono tabular-nums">{`${pointText(item, ['entering_cooling_water_temperature', '冷却水进水'], 1).replace(/ °C$/, '')} / ${pointText(item, ['leaving_cooling_water_temperature', '冷却水出水'], 1).replace(/ °C$/, '')}`}</TableCell>
                      </TableRow>
                    );
                  })}
                </TableBody>
      ...
      <section className="hvac-plant-ref__bottom-grid">
        ...
        <PlantPanel title="设备运行对比（实时）">
          <Table aria-label="设备运行对比">
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  <TableHead>设备类型</TableHead>
                  <TableHead>设备编号</TableHead>
                  <TableHead className="w-28 text-center">运行状态</TableHead>
                  <TableHead className="text-right">频率 / 负荷率</TableHead>
                  <TableHead className="text-center">关键指标趋势（30分钟）</TableHead>
                  <TableHead className="text-right">今日累计运行</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {comparisonRows.map((row) => (
                  <TableRow
                    key={row.key}
                    className={row.device ? 'cursor-pointer' : undefined}
                    onClick={(event) => {
                      if (row.device && !(event.target as HTMLElement).closest('button,a')) onOpenDevice(row.device.device.id);
                    }}
                  >
                    <TableCell className="font-medium">{row.type}</TableCell>
                    <TableCell>{row.label}</TableCell>
                    <TableCell className="text-center">{row.state}</TableCell>
                    <TableCell className="text-right font-mono tabular-nums">{row.operating}</TableCell>
                    <TableCell className="text-center"><span className="hvac-plant-ref__mini-trend is-empty">趋势未返回</span></TableCell>
