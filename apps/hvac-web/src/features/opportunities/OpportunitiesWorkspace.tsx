...
const OPPORTUNITY_STATUS_OPTIONS = Object.entries(STATUS_LABELS).map(([value, meta]) => ({ value, label: meta.label }));
const OPPORTUNITY_COMPLEXITY_OPTIONS = Object.entries(COMPLEXITY_LABELS).map(([value, meta]) => ({ value, label: meta.label }));
const OPPORTUNITY_SUBSYSTEM_OPTIONS = Object.entries(SUBSYSTEM_LABELS).map(([value, label]) => ({ value, label }));
const OPPORTUNITY_FILTER_COLUMN_IDS = ['status', 'complexity', 'subsystem'] as const;
const OPPORTUNITY_FILTERS_QUERY_KEY = 'opportunityFilters';
const OPPORTUNITY_JOIN_OPERATOR_QUERY_KEY = 'opportunityJoinOperator';

function matchesOpportunityAdvancedFilter(
  item: EcoOpportunity,
  filter: ExtendedColumnFilter<EcoOpportunity>,
) {
...
export function OpportunitiesWorkspace({ site }: OpportunitiesProps) {
  ...
  const [advancedFilters] = useQueryState(
    OPPORTUNITY_FILTERS_QUERY_KEY,
    getFiltersStateParser<EcoOpportunity>([...OPPORTUNITY_FILTER_COLUMN_IDS]).withDefault([]),
  );
  const [joinOperator] = useQueryState(
    OPPORTUNITY_JOIN_OPERATOR_QUERY_KEY,
  ...
  const filteredData = useMemo(() => {
    return OPPORTUNITIES_DATA.filter((item) => {
      ...
      const filterMatches = advancedFilters.map((filter) => matchesOpportunityAdvancedFilter(item, filter));
      const matchAdvancedFilters = advancedFilters.length === 0 || (joinOperator === 'or'
        ? filterMatches.some(Boolean)
        : filterMatches.every(Boolean));
      return matchSearch && matchAdvancedFilters;
    });
  }, [advancedFilters, joinOperator, search]);
  ...
  const columns = useMemo<Array<ColumnDef<DataTableFeatures, EcoOpportunity>>>(() => [
    ...
    { id: 'code', accessorFn: (row) => row.code, meta: { label: '编号' }, header: '编号', cell: ({ row }) => <span className="font-mono text-xs text-muted-foreground">{row.original.code}</span> },
    {
      id: 'title',
    ...
    { id: 'subsystem', accessorFn: (row) => row.subsystem, enableColumnFilter: true, meta: { label: '所属系统', variant: 'select', options: OPPORTUNITY_SUBSYSTEM_OPTIONS }, header: '所属系统', cell: ({ row }) => <span className="text-sm">{SUBSYSTEM_LABELS[row.original.subsystem]}</span> },
    {
      id: 'complexity',
      accessorFn: (row) => row.complexity,
      enableColumnFilter: true,
      meta: { label: '实施难度', variant: 'select', options: OPPORTUNITY_COMPLEXITY_OPTIONS },
      header: '实施难度',
      cell: ({ row }) => (
        <StatusBadge
    ...
    { id: 'savingsKWh', accessorFn: (row) => row.annualSavingsKWh, meta: { label: '预期年节电' }, header: '预期年节电', cell: ({ row }) => <span className="font-mono text-sm font-medium tabular-nums">{row.original.annualSavingsKWh.toLocaleString()} kWh</span> },
    { id: 'savingsCost', accessorFn: (row) => row.annualSavingsCostCNY, meta: { label: '预期年节费' }, header: '预期年节费', cell: ({ row }) => <span className="font-mono text-sm font-semibold text-foreground tabular-nums">¥{row.original.annualSavingsCostCNY.toLocaleString()}</span> },
    { id: 'payback', accessorFn: (row) => row.paybackMonths, meta: { label: '回收期' }, header: '回收期', cell: ({ row }) => <span className="font-mono text-sm tabular-nums">{row.original.paybackMonths === 0 ? '即时/零成本' : `${row.original.paybackMonths} 个月`}</span> },
    {
      id: 'status',
      accessorFn: (row) => row.status,
      enableColumnFilter: true,
      meta: { label: '推进状态', variant: 'select', options: OPPORTUNITY_STATUS_OPTIONS },
      header: '推进状态',
      cell: ({ row }) => (
        <StatusBadge
  ...
  const table = useDataTable({
    ...
    meta: {
      queryKeys: {
        ...
        perPage: 'opportunityPerPage',
        sort: 'opportunitySort',
        filters: OPPORTUNITY_FILTERS_QUERY_KEY,
        joinOperator: OPPORTUNITY_JOIN_OPERATOR_QUERY_KEY,
      },
    },
  });
  ...
  return (
    <Main className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="shadow-xs">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">预计年节电</CardTitle>
      ...
      <Sheet open={Boolean(selectedOpportunity)} onOpenChange={(open) => { if (!open) setSelectedOpportunity(null); }}>
        <SheetContent className="w-full sm:max-w-2xl! overflow-y-auto">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4 space-y-6 pt-4">
            {selectedOpportunity ? (
