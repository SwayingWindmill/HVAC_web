import type { Command, CommandStatus } from '@/api/command-contract';

export type CommandBusinessState =
  | 'IN_PROGRESS'
  | 'AWAITING_APPROVAL'
  | 'SUCCEEDED'
  | 'FAILED'
...
export interface CommandProjection {
  readonly businessState: CommandBusinessState;
...
export function projectCommand(command: Command): CommandProjection {
  const canApprove =
    command.status === 'AWAITING_APPROVAL' &&
    command.approvalCount < command.requiredApprovalCount;

  const businessState: CommandBusinessState =
    command.status === 'AWAITING_APPROVAL'
      ? 'AWAITING_APPROVAL'
