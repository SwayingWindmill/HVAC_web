...
export interface AssetsPointDefinition {
  readonly key: string;
...
export interface AssetsDeviceProfile {
  readonly kind: string;
  readonly title: string;
  readonly aliases: readonly string[];
  readonly points: readonly AssetsPointDefinition[];
}

export type AssetsProfileResolution =
  | { readonly state: 'configured'; readonly profile: AssetsDeviceProfile }
...
const point = (
  ...
  label: string,
  order: number,
  options: Partial<Omit<AssetsPointDefinition, 'key' | 'label' | 'order'>> = {},
): AssetsPointDefinition => Object.freeze({
  key,
  label,
  order,
...
const profile = (
  ...
  points: readonly AssetsPointDefinition[],
): AssetsDeviceProfile => Object.freeze({
  kind,
  title,
...
] satisfies readonly AssetsDeviceProfile[]);

const PROFILE_BY_ALIAS = new Map<string, AssetsDeviceProfile>(
  REAL_ASSETS_DEVICE_PROFILES.flatMap((definition) => definition.aliases.map((alias) => [alias, definition] as const)),
);

export function normalizeAssetsDeviceType(deviceType: string | null | undefined): string {
  return (deviceType ?? '').trim().toUpperCase().replace(/[^A-Z0-9]+/g, '_').replace(/^_+|_+$/g, '');
}

export function resolveAssetsProfile(deviceType: string | null | undefined): AssetsProfileResolution {
  const normalizedDeviceType = normalizeAssetsDeviceType(deviceType);
...
export function listPointDefinitions(resolution: AssetsProfileResolution): readonly AssetsPointDefinition[] {
