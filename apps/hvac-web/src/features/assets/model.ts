...
import {
  ...
  type AssetsProfileResolution,
} from './catalog.ts';
import {
  projectAssetsDeviceOperationalState,
  type AssetsAttentionReason,
  type AssetsDeviceOperationalProjection,
  type AssetsPointView,
  type AssetsSnapshotResult,
} from './operational-projection.ts';

export type { AssetsAttentionReason, AssetsPointView, AssetsSnapshotResult } from './operational-projection.ts';

export type AssetsBindingState =
...
export type AssetsSpaceState =
  | { readonly state: 'bound'; readonly relationship: AssetRelationship; readonly space: Space }
  | { readonly state: 'unbound' }
  | { readonly state: 'ambiguous'; readonly relationshipIds: readonly string[] };

export interface AssetsDeviceRow {
  readonly device: Device;
  readonly profile: AssetsProfileResolution;
  readonly binding: AssetsBindingState;
  readonly space: AssetsSpaceState;
  readonly registeredPointCount: number;
  readonly telemetryPoints: readonly TelemetryPoint[];
  readonly snapshotResult?: AssetsSnapshotResult;
  readonly operational: AssetsDeviceOperationalProjection;
  readonly representativePoints: readonly AssetsPointView[];
}

export interface AssetsAssetRow {
  readonly asset: Asset;
  readonly space: AssetsSpaceState;
  readonly devices: readonly AssetsDeviceRow[];
  readonly sensors: readonly Sensor[];
  readonly points: readonly AssetsTelemetryPointRow[];
  readonly controlPoints: readonly AssetsTelemetryPointRow[];
  readonly attentionReasons: readonly AssetsAttentionReason[];
...
export interface BuildAssetsRowsInput {
  readonly assetModel: SiteAssetModel;
  readonly snapshots?: ReadonlyMap<string, AssetsSnapshotResult>;
  readonly now?: Date;
}

export type AssetsHierarchyKind = 'site' | 'space' | 'asset' | 'device';

export interface AssetsHierarchyNode {
  readonly key: string;
  readonly kind: AssetsHierarchyKind;
  readonly label: string;
  readonly meta: string;
  readonly deviceIds: readonly string[];
  readonly children: readonly AssetsHierarchyNode[];
}

export interface AssetsTelemetryPointRow {
...
export interface BuildAssetsAssetRowsInput {
  readonly assetModel: SiteAssetModel;
  readonly deviceRows: readonly AssetsDeviceRow[];
  readonly pointRows: readonly AssetsTelemetryPointRow[];
  readonly now?: Date;
}
...
const REGISTRY_STATUS_LABELS: Readonly<Record<string, string>> = Object.freeze({
  ACTIVE: '启用',
  INACTIVE: '停用',
  RETIRED: '已退役',
});
...
export function assetsSpaceTypeLabel(value: string): string {
...
export function assetsSensorTypeLabel(value: string): string {
  return localizedType(value, SENSOR_TYPE_LABELS, '传感器');
}

export function assetsPointTypeLabel(value: string): string {
...
export function assetsTelemetryPointMeta(point: TelemetryPoint): string {
  ...
  return unit ? `${assetsPointTypeLabel(point.pointType)} · ${unit}` : assetsPointTypeLabel(point.pointType);
}
...
function resolvedAssetBindings(binding: AssetsBindingState): readonly { readonly relationship: AssetRelationship; readonly asset: Asset }[] {
...
export function selectAssetsRepresentativePoints(
  telemetryPoints: readonly TelemetryPoint[],
  profile: AssetsProfileResolution,
  points: readonly AssetsPointView[],
): readonly AssetsPointView[] {
  const pointCodeBySourceKey = new Map(telemetryPoints.map((point) => [point.sourceKey, point.pointCode] as const));
  const preferred = listPointDefinitions(profile)
    .map((definition) => pointCodeBySourceKey.get(definition.key))
    .filter((pointCode): pointCode is string => Boolean(pointCode));
  ...
  return [...preferred, ...generic]
    ...
    .filter((point): point is AssetsPointView => Boolean(point));
}

export function buildAssetsRows(input: BuildAssetsRowsInput): AssetsDeviceRow[] {
  ...
  const rows = input.assetModel.devices.map((device): AssetsDeviceRow => {
    const profile = resolveAssetsProfile(device.deviceType);
...
export function buildAssetsAssetRows(input: BuildAssetsAssetRowsInput): AssetsAssetRow[] {
  const now = input.now ?? new Date();
  ...
  const deviceRowsByAsset = new Map<string, AssetsDeviceRow[]>();
  ...
  return input.assetModel.assets.map((asset): AssetsAssetRow => {
...
function hierarchyNode(
  kind: AssetsHierarchyKind,
  id: string,
  label: string,
  meta: string,
  deviceIds: readonly string[],
  children: readonly AssetsHierarchyNode[] = [],
): AssetsHierarchyNode {
  return {
    key: `${kind}:${id}`,
    kind,
...
export function buildAssetsHierarchy(model: SiteAssetModel, siteLabel: string, now = new Date()): AssetsHierarchyNode {
  ...
  const deviceNode = (device: Device, parentKey: string): AssetsHierarchyNode => hierarchyNode(
    'device',
    `${parentKey}:${device.id}`,
    device.displayName,
    `${assetsDeviceTypeLabel(device.deviceType)} · ${device.code}`,
    [device.id],
  );

  const assetNode = (asset: Asset): AssetsHierarchyNode => {
    ...
    return hierarchyNode(
      ...
      asset.id,
      asset.displayName,
      `${assetsAssetTypeLabel(asset.assetType)} · ${asset.code}`,
  ...
  const spaceNode = (space: Space): AssetsHierarchyNode => {
    ...
    return hierarchyNode(
      ...
      space.id,
      space.displayName,
      `区域 · ${assetsSpaceTypeLabel(space.spaceType)}`,
