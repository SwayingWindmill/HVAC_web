...
export function DataTableFilterList<TData extends RowData>({
  ...
  const resetPage = React.useCallback(() => table.setPageIndex(0), [table]);
  ...
  }, [columns, filters, debouncedSetFilters, resetPage]);
  ...
  }, [resetPage, setFilters, setJoinOperator]);

  const onFiltersReorder = React.useCallback((nextFilters: ExtendedColumnFilter<TData>[]) => {
    resetPage();
    void setFilters(nextFilters);
  }, [resetPage, setFilters]);

  const onJoinOperatorChange = React.useCallback((value: JoinOperator) => {
    resetPage();
  ...
  return (
    <Sortable
      value={filters}
      onValueChange={onFiltersReorder}
          ...
          <PopoverTrigger asChild>
            ...
            >
              <span className="truncate">
                {columns.find((candidate) => candidate.id === filter.id)
              ...
              <CommandList>
                ...
                <CommandGroup>
                  {columns.map((candidate) => (
                    <CommandItem
                      ...
                      onSelect={(value) => {
                        onFilterUpdate(filter.filterId, {
                          ...
                          variant: candidate.columnDef.meta?.variant ?? "text",
                          operator: getDefaultFilterOperator(
                            candidate.columnDef.meta?.variant ?? "text",
                          ),
                          value: "",
                        });
                    ...
                    >
                      <span className="truncate">
                        {candidate.columnDef.meta?.label}
                      </span>
                      <Check
                        className={cn(
                          "ml-auto",
