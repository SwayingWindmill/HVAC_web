...
`energy-api` is the canonical public Go application entrypoint. Its `internal/gateway` package owns browser-facing HTTP composition, BFF Session handling, authorization/delegation at the public edge, and routing into domain owners. Domain state and business authority remain in `modules/*` or deliberately independent workloads; `energy-api` is not a separate business domain.
...
```bash
go build -ldflags "-X main.version=0.3.0 -X main.commit=$(git rev-parse HEAD) -X main.builtAt=$(date -u +%Y-%m-%dT%H:%M:%SZ)" ./cmd/energy-api
```
