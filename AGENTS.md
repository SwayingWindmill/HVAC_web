...
- Keep the configured required-check set small and stable. Branch rules, workflow names and documentation must agree. Never merge while a required check is pending or failed. A failing non-required check must still be investigated; do not relabel a real failure as optional to get a merge through.
