...
- 空状态按当前路由和对象展示分析范围、3 条页面操作和最近会话；不得把 Popup 做成缩小版 AI 首页，也不得重复展示页面已经可见的全局指标。
- Header 中的历史入口必须在当前 Popup 内切换到会话列表，支持搜索和恢复；打开 Popup 后隐藏右下角启动按钮，避免出现两个 AI 入口。
...
```text
AI Hub（固定视口）
├── Thread Navigator      搜索、筛选、分页、固定、归档
├── CopilotChat Pane
│   ├── 会话标题 / 新建会话
│   ├── 当前上下文条
│   ├── 官方消息与 Generative UI 区  ← 唯一纵向滚动区域
│   └── 官方输入区与免责声明
└── Evidence Rail
    ├── 当前线程元数据
    ├── 运行态与关键读数
...
- 桌面端采用左侧线程导航、中间主对话、右侧证据栏三列结构；小屏幕将左右栏折叠为 Drawer，但主页面仍保持固定视口。
- 工作台不得把右侧每个信息组包装成独立 Card，也不得在顶部叠加整条彩色“AI 安全说明”卡。
- 对话是页面主任务，桌面端获得剩余主宽度；左右栏是稳定导航和证据区域，不得随长对话一起移动。
