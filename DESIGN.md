...
- 中央 3D 默认使用固定等轴视图，允许有限角度拖拽但禁止自动旋转；设备详情不得使用与模型脱节的绝对定位浮层，统一进入主视图下方的设备状态带。冷站模型需使用可辨识的工业设备结构：双筒式冷机、泵电机与蜗壳、方形逆流冷却塔、负荷模块、法兰、阀门和分区底座，并通过金属度、粗糙度、轮廓、状态灯与业务色表达层级；不得退回方盒、单圆柱和无连接细节的占位几何。
