...
- 桌面端使用无遮罩、非模态的右侧浮动工作面板；页面必须保持可见、可滚动、可点击，AI 不得以整屏遮罩阻断当前业务操作。
- 标准宽度为 600px，聚焦宽度为 860px；面板与顶部、右侧、底部保留可见间距，使用独立圆角、边框与阴影建立窗口层级，而不是贴边满高 Drawer。
- 面板支持最小化、恢复、标准/聚焦切换、打开完整工作台和关闭。最小化只能收起窗口，不得清空会话、输入草稿或运行状态。
- 手机端使用全屏 Agent Sheet，不保留桌面浮动间距；不得产生横向溢出。
- 空闲首页优先展示当前页面/对象上下文、待关注事项和上下文动作，不使用大面积欢迎语或通用热点内容。
- 生成中只展示真实发生的上下文读取、业务摘要和回答生成状态；未接入工具 Runtime 前不得伪造多工具调查步骤。
