...
CREATE TABLE IF NOT EXISTS iam.alarm_permissions (
  ...
  action text NOT NULL CHECK (action IN ('alarm:list', 'alarm:read')),
  ...
  UNIQUE (principal_id, acting_organization_id, site_id, action, effect),
...
CREATE TABLE IF NOT EXISTS iam.alarm_authorization_decisions (
  ...
  CHECK ((action = 'alarm:list' AND alarm_id IS NULL) OR (action = 'alarm:read' AND alarm_id IS NOT NULL))
);

CREATE INDEX IF NOT EXISTS alarm_permissions_lookup_idx
  ON iam.alarm_permissions (principal_id, acting_organization_id, site_id, action, effect);
CREATE INDEX IF NOT EXISTS alarm_authorization_decisions_scope_idx
  ON iam.alarm_authorization_decisions (acting_organization_id, occurred_at DESC, sequence DESC);
...
CREATE POLICY alarm_authorization_decisions_runtime_insert ON iam.alarm_authorization_decisions
  ...
  WITH CHECK (
    acting_organization_id = iam.current_acting_organization_id()
...
VALUES
  ('018f1e00-2400-7000-8000-000000000021', '018f1e00-2000-7000-8000-000000000001', '018f1e00-0000-7000-8000-000000000001', '018f1e00-1000-7000-8000-000000000001', 'alarm:list', 'ALLOW', 'ACTIVE', '2026-08-01T00:00:00Z', NULL, 1, '2026-08-01T00:00:00Z', '2026-08-01T00:00:00Z'),
  ('018f1e00-2400-7000-8000-000000000022', '018f1e00-2000-7000-8000-000000000001', '018f1e00-0000-7000-8000-000000000001', '018f1e00-1000-7000-8000-000000000001', 'alarm:read', 'ALLOW', 'ACTIVE', '2026-08-01T00:00:00Z', NULL, 1, '2026-08-01T00:00:00Z', '2026-08-01T00:00:00Z')
ON CONFLICT DO NOTHING;
