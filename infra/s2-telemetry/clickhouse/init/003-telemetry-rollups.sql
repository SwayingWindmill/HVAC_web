...
ORDER BY (tenant_id, owning_organization_id, site_id, point_id, sensor_id, device_id, telemetry_key, unit, bucket)
...
SELECT
  ...
  assumeNotNull(owning_organization_id) AS owning_organization_id,
...
GROUP BY bucket, tenant_id, owning_organization_id, site_id, device_id, point_id, sensor_id, telemetry_key, unit;
...
CREATE TABLE IF NOT EXISTS telemetry_history.numeric_daily_states (
  bucket DateTime('UTC'),
  tenant_id UUID,
  owning_organization_id UUID,
  site_id UUID,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS telemetry_history.observations_to_numeric_daily
TO telemetry_history.numeric_daily_states
AS
SELECT
  toStartOfDay(sampled_at) AS bucket,
  assumeNotNull(tenant_id) AS tenant_id,
  assumeNotNull(owning_organization_id) AS owning_organization_id,
  assumeNotNull(site_id) AS site_id,
  assumeNotNull(device_id) AS device_id,
...
CREATE VIEW IF NOT EXISTS telemetry_history.numeric_daily AS
SELECT
  bucket,
  tenant_id,
  owning_organization_id,
  ...
  minMerge(minimum_value) AS minimum_value,
  maxMerge(maximum_value) AS maximum_value
FROM telemetry_history.numeric_daily_states
GROUP BY bucket, tenant_id, owning_organization_id, site_id, device_id, point_id, sensor_id, telemetry_key, unit;
