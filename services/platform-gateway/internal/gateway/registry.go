...
func validateSiteAssetModel(value platformapi.SiteAssetModel, expectedSiteID string) error {
	if value.SchemaVersion != 1 || value.SiteID != expectedSiteID || !isLowerUUIDv7(expectedSiteID) {
	...
	for _, item := range value.Equipment {
		if err := validateEquipment(item); err != nil || item.SiteID != expectedSiteID {
	...
	for _, item := range value.Devices {
		if err := validateDevice(item); err != nil || item.SiteID != expectedSiteID {
	...
	for _, input := range value.CalculatedPointInputs {
		if input.SiteID != expectedSiteID || !isLowerUUIDv7(input.OwningOrganizationID) || input.Ordinal < 0 || !validRegistryString(input.InputRole, 128) || !validRegistryString(input.FormulaRevision, 256) {
