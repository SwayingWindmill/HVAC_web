...
CREATE TABLE IF NOT EXISTS alarm_runtime.alarm_idempotency (
  ...
  PRIMARY KEY (organization_id, site_id, alarm_id, idempotency_key),
  FOREIGN KEY (organization_id, site_id, alarm_id)
    REFERENCES alarm_runtime.alarm_current (organization_id, site_id, alarm_id)
    ON DELETE CASCADE
);
