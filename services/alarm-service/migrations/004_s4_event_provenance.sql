...
CREATE TABLE alarm_runtime.events (
  ...
  UNIQUE (tenant_id, organization_id, site_id, event_id),
  CHECK (end_time IS NULL OR end_time >= start_time)
);

CREATE INDEX event_site_time_idx
  ON alarm_runtime.events (tenant_id, organization_id, site_id, start_time DESC, event_id);
CREATE INDEX event_device_time_idx
  ON alarm_runtime.events (tenant_id, organization_id, site_id, device_id, start_time DESC)
  WHERE device_id IS NOT NULL;
CREATE INDEX event_point_time_idx
  ON alarm_runtime.events (tenant_id, organization_id, site_id, point_id, start_time DESC)
...
ALTER TABLE alarm_runtime.alarm_current
  ...
  ADD CONSTRAINT alarm_current_event_scope_fk
    FOREIGN KEY (tenant_id, organization_id, site_id, event_id)
    REFERENCES alarm_runtime.events (tenant_id, organization_id, site_id, event_id);

CREATE INDEX alarm_current_event_idx
  ON alarm_runtime.alarm_current (tenant_id, organization_id, site_id, event_id)
  WHERE event_id IS NOT NULL;
CREATE INDEX alarm_current_point_idx
  ON alarm_runtime.alarm_current (tenant_id, organization_id, site_id, point_id, last_occurred_at DESC)
  WHERE point_id IS NOT NULL;
