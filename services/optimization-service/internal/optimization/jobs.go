...
func (store *PostgresStore) ClaimOptimizationJobs(ctx context.Context, workerID string, batch int, leaseDuration time.Duration, now time.Time) ([]SchedulerJob, error) {
	if store == nil || store.pool == nil {
		return nil, errors.New("optimization postgres store is unavailable")
	}
	if workerID == "" || batch <= 0 || batch > 100 || leaseDuration <= 0 {
		return nil, errors.New("optimization job claim parameters are invalid")
	}
...
WHERE state='READY' AND cancel_requested=false AND tenant_id IS NOT NULL AND site_id IS NOT NULL AND job_type='OPTIMIZATION_RUN'
...
WHERE job_id=$1::uuid AND state='READY'`, job.JobID, workerID, now.UTC().Add(leaseDuration), now.UTC())
		...
		if tag.RowsAffected() != 1 {
			return nil, errors.New("optimization job claim lost after row lock")
		}
	}
...
func (store *PostgresStore) StartOptimizationJob(ctx context.Context, job SchedulerJob, leaseDuration time.Duration, now time.Time) (bool, error) {
	attemptID, err := optimizationUUIDv7(now)
	if err != nil {
		return false, err
	}
	tx, err := store.pool.Begin(ctx)
...
WHERE job_id=$1::uuid AND state='CLAIMED' AND lease_owner=$2`, job.JobID, job.WorkerID, job.AttemptNo, now.UTC(), leaseDuration.String())
	...
	if tag.RowsAffected() != 1 {
		return false, errors.New("optimization job RUNNING transition rejected")
	}
	return true, tx.Commit(ctx)
}

func (store *PostgresStore) RenewOptimizationJobLease(ctx context.Context, job SchedulerJob, leaseDuration time.Duration, now time.Time) (bool, error) {
	var cancelRequested bool
...
RETURNING cancel_requested`, job.JobID, job.WorkerID, now.UTC().Add(leaseDuration), now.UTC()).Scan(&cancelRequested)
	if errors.Is(err, pgx.ErrNoRows) {
		return false, errors.New("optimization job lease ownership was lost")
	}
	return cancelRequested, err
}

func (store *PostgresStore) CompleteOptimizationJob(ctx context.Context, job SchedulerJob, output map[string]any, now time.Time) error {
...
WHERE job_id=$1::uuid AND state='RUNNING' AND lease_owner=$2`, job.JobID, job.WorkerID, now.UTC())
	...
	if tag.RowsAffected() != 1 {
		return errors.New("optimization job success transition rejected")
	}
...
func (store *PostgresStore) FailOptimizationJob(ctx context.Context, job SchedulerJob, errorCode string, executionErr error, retryable bool, now time.Time) error {
	if errorCode == "" {
		errorCode = "OPTIMIZATION_JOB_FAILED"
	}
	message := errorCode
	if executionErr != nil {
	...
	if retryable && job.AttemptNo < job.MaxAttempts {
		...
		nextRetry = now.UTC().Add(optimizationRetryDelay(job.AttemptNo))
		completed = nil
...
WHERE job_id=$1::uuid AND state='RUNNING' AND lease_owner=$2`, job.JobID, job.WorkerID, state, nextRetry, completed, errorCode, message, now.UTC())
	...
	if tag.RowsAffected() != 1 {
		return errors.New("optimization job failure transition rejected")
	}
	if _, err = tx.Exec(ctx, `UPDATE core_registry.job_attempts
