...
| --- | --- | --- | --- | --- |
| Deterministic TCP dependency failures | `Shopify/toxiproxy` | `v2.12.0`, OCI digest `sha256:9378ed52a28bc50edc1350f936f518f31fa95f0d15917d6eb40b8e376d1a214e` | MIT | Used for PostgreSQL availability and Legacy latency/circuit tests. |
...
- Browser-originated traffic reaches only Gateway; IAM, Audit, PostgreSQL, Redpanda and Legacy are denied.
- Gateway egress to metadata address `169.254.169.254` and private DNS-rebinding target `10.0.0.1` is denied.
- Toxiproxy produces explicit PostgreSQL-unavailable and Legacy timeout/circuit-open states without leaking internal details.
...
- Route ownership rollback remains rejected and auditable.
