...
Dependency security is now a stable capability workflow rather than a Ticket wrapper. `.github/workflows/dependency-security.yml` runs only production dependency audit, license validation and the retry harness when dependency manifests or those audit tools change. RMS/S1/S2 build and browser suites are owned by their own PR/nightly profiles, and the React Router security test protects patched versions/imports rather than testing CI path-list implementation details.
...
The S2 topology migration replaced numbered Ticket entry points with capability-oriented commands and evidence directories. RC-05 has since retired the Runtime Snapshot and Gateway Snapshot workflows because they only re-validated source shape and orchestration already covered by contracts, ownership, Go tests and integration checks. The remaining S2 topology gate protects current capability/workflow mappings and rejects restored Ticket wrappers; operational evidence remains capability-scoped, while historical snapshot definitions live only as retired evidence.
...
The former S3 Ticket 09 certification automation has been retired. S3 no longer has a certification preflight, formal evidence bundle or aggregate release blocker; runtime validation is invoked explicitly and kept focused on the changed command path.

The remaining S3 capability names are historical organization aids only. Native MQTT command execution is the current provider path, and obsolete numbered/certification wrappers are not restored.
...
`.github/workflows/nightly-full-regression.yml` runs daily at 18:00 UTC and supports manual dispatch. It reuses `scripts/run-pr-gate.mjs` to execute the complete static, contract, unit, integration and browser profile sets across Linux and Windows. The workflow names profile sets instead of copying profile lists: `all` covers every profile for a gate, while `browser-linux` and `browser-windows` preserve the runner boundary. `scripts/run-pr-gate.mjs` and the local `scripts/run-domain-task.mjs` runner both resolve commands and profile sets from `scripts/domain-task-matrix.mjs`, so PR, nightly and developer execution cannot silently drift into separate catalogs. Matrix initialization fails if a supported profile is omitted, duplicated or assigned to neither browser platform. Pull requests remain affected-path selective, while the nightly workflow provides cross-domain coverage, including the Operations Agent service checks, benchmark tests and PostgreSQL integration. Static, integration and browser evidence is uploaded with a 14-day retention period so failures remain inspectable after ephemeral runners are removed.

Long capability aggregations keep their stable npm names but delegate to `scripts/run-capability-task.mjs`; command order is versioned in the same matrix and verified through dry-run tests. `scripts/package-script-long-chain-baseline.json` is a ratchet for remaining legacy inline chains: scripts with more than four `&&`-separated commands may remain only while their exact SHA-256 value matches the reviewed baseline. New chains, modified chains, stale exemptions and capability tasks that stop delegating all fail `repo:check`. Baseline regeneration is therefore an explicit review action, not a routine fix for governance failures.
