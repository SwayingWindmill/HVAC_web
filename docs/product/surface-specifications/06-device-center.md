...
> **状态：SELECTED / IMPLEMENTED WORKSPACE BASELINE**  
> **日期：2026-09-14**  
...
## 3.10 Johnson Controls Metasys — Equipment Dashboard + Custom Trend Viewer 的双层详情

Metasys UI 16.0 的 Equipment Dashboard 将 Trend、Equipment Activity、Equipment Data、Relationships、Graphics、Schedule 放在同一设备上下文；同时又提供跨设备 Custom Trend Viewer。
...
- Device Inspector 先给当前设备所需的快速事实；
- 长历史、跨设备趋势不塞进 Inspector，而进入 Advanced Trend Studio；
- Durable Device Detail 继续承担完整工程上下文。

## 3.11 Primary-detail / Peek — 列表保持主任务，Preview 不升级成第二个详情页

成熟 primary-detail 模式允许用户在大列表中查看选中对象，同时继续保留列表上下文。PatternFly 明确区分 overlay drawer 与会压缩主内容的 inline drawer，并指出只有详情确实需要更多空间时才需要 splitter；Linear Peek 则把 preview 定义成“不进入完整对象页面即可快速查看”，并支持在列表中连续切换对象。Radix Dialog 原语本身支持 modal / non-modal 两种模式。

来源：

- https://www.patternfly.org/components/drawer/design-guidelines/
- https://www.patternfly.org/patterns/primary-detail/
...
- Wide desktop：DataTable 始终保持完整宽度；selection 打开固定宽度 non-modal Quick Preview，不通过 splitter 改变 Ledger 几何；
- Preview 无遮罩、不 trap focus，用户仍可在列表可见区域选择另一设备并原位更新 Preview；
- Narrow / mobile：同一份 Quick Preview 使用 modal Sheet；
- Preview 只承担快速判断，不包含 point tabs、relationship tabs、完整历史或复杂控制；
- `inspect` 可以进入 URL 表达被检查对象；Preview 宽度、动画和 overlay 状态不进入 URL。
...
`inspect` 仅用于可恢复的 Context Inspector selection；它表达被检查的设备，不表达 modal open animation。
...
| 对象与位置 | authoritative system / asset relationship + concise location |
| 运行 | Operating state |
...
- selected 时可打开 Quick Preview，但 Ledger 宽度不得因此变化。

页面主要视觉面积属于 Ledger，不属于 KPI cards；Preview 覆盖右侧部分内容但不触发 table reflow。

## 1024–1439 px

- Toolbar 可 wrap；
- System / Type 等 filter 保持可达；
- 默认转为 Sheet；不在中等宽度强行挤出双栏 Inspector；
...
Quick Preview                → shadcn Sheet/Dialog primitive: non-modal overlay (wide) / modal Sheet (narrow)
...
- wide desktop Quick Preview 为 non-modal overlay，打开前后 Ledger 宽度保持不变，且没有 resizable handle；
- narrow Sheet 遵循 modal focus management；
- realtime 不抢 focus。
...
- Header / Toolbar / Ledger 构成稳定主工作区；Quick Preview 是按需覆盖层；
- Ledger 是主要视觉面积，Preview 不触发布局重排；
