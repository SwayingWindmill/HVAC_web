...
- `welcomeScreen`: ChatGPT-inspired quiet empty state with one route-aware context line, large flexible whitespace, three recent sessions, and a refined 76px-height, 18px-radius composer with a clear border, restrained two-level shadow, 3px focus ring, and 40px send action; unavailable add-content controls leave the layout instead of consuming space; it intentionally omits the welcome hero, page description, scope card, suggestion list, duplicated metrics, and nested cards;
...
- `/ai` workspace: a full-bleed AppShell workspace with no page Hero or outer Card; it keeps the shared operations colors and typography while thread navigator, primary official CopilotChat pane, and evidence inspector are separated only by 1px dividers; the primary composer is always anchored to the bottom of the center pane, while empty-state guidance and suggestions live in a separately scrollable region above it. Guidance, suggestion actions, and Composer share the same centered 760px content axis. Suggestions are vertically stacked, single-line, content-width actions with a subtle surface, 12px radius, and right arrow rather than full-width divider rows or cards. The Composer is about 78px high and uses an 18px radius, clear 3px focus ring, hidden unavailable add action, and 42px send action. Before the first message the evidence rail shows only investigation context and available data; runtime snapshots and handoffs appear after the investigation starts;
- `labels`: Chinese title, dynamic input placeholder, and toolbar labels;
- persistent input disclaimers are disabled so the Composer remains visually continuous; safety boundaries stay in the surrounding investigation context and business workflow;
