...
- productized official CopilotKit `CopilotPopup`: compact solid launcher and header, page-aware operations, three recent sessions, in-panel history search, historical message restoration, duplicate-launcher suppression while open, new-session behavior, local self-managed Agent streaming, all three Generative UI evidence panels, desktop/mobile geometry, opaque light/dark surfaces, and zero horizontal overflow across history, messages, results, composer, Markdown, code, and tables;
- official `/ai` `CopilotChat` workspace: shared Agent/thread/context with the Popup, fixed browser/AppShell/workspace viewport, left thread navigator, center message workspace, right evidence rail, search and historical investigation restore, localStorage persistence across reload, new-thread action, Generative UI rendering, desktop/mobile responsive geometry, zero page-level vertical scrolling, vertical scrolling only inside the center message viewport, and zero horizontal overflow;
