...
- productized official CopilotKit `CopilotPopup`: compact solid launcher and header, live presence and attention count, route/object/period-aware operational brief, flat scope and attention rows, suggestions and placeholder, new-session behavior, local self-managed Agent streaming, all three Generative UI evidence panels, desktop/mobile geometry, opaque light/dark surfaces, removal of the old equal-weight metric-strip template, and zero horizontal overflow across messages, results, composer, Markdown, code, and tables;
- official `/ai` `CopilotChat` workspace: shared Agent/thread/context with the Popup, single main conversation pane, one continuous runtime/evidence/handoff rail instead of nested Cards, embedded operational welcome and composer, new-thread action, Generative UI result rendering, desktop/mobile responsive geometry, and zero horizontal overflow;
