...
- productized official CopilotKit `CopilotPopup`: low-weight 120×40 launcher, 520×680 desktop geometry, 16px Surface radius, exactly three header actions, no duplicate read-only/expand controls, page-aware operations, two recent sessions, in-panel history search, historical message restoration, duplicate-launcher suppression while open, local self-managed Agent streaming, Generative UI evidence panels with no custom business text below 11px, mobile full-screen geometry, opaque light/dark surfaces, and zero horizontal overflow;
- official `/ai` `CopilotChat` workspace: shared Agent/thread/context with the Popup, shared PageScaffold and `--ops-*` tokens, fixed browser/AppShell/workspace viewport, one 16px Raised Surface with zero column gap and divider-only thread/conversation/evidence regions, center conversation at least 1.5× either side rail, search and historical investigation restore, localStorage persistence across reload, Generative UI rendering, no custom business text below 11px, zero page-level vertical scrolling, vertical scrolling only inside the center message viewport, and zero horizontal overflow;
