...
type counterDeltaRow struct {
	...
	ExistingFactFound                 uint8    `json:"existing_fact_found"`
	ExistingFactMeterBindingID        string   `json:"existing_fact_meter_binding_id"`
	ExistingFactPreviousObservationID string   `json:"existing_fact_previous_observation_id"`
	ExistingFactRevision              uint64   `json:"existing_fact_revision"`
}
...
func (reader *Reader) ListDeltas(ctx context.Context, limit int) ([]energy.CounterDelta, error) {
	...
	for {
		...
		deltas = append(deltas, energy.CounterDelta{
			...
			ExistingFactFound:                 row.ExistingFactFound != 0,
			ExistingFactMeterBindingID:        row.ExistingFactMeterBindingID,
			ExistingFactPreviousObservationID: row.ExistingFactPreviousObservationID,
			ExistingFactRevision:              row.ExistingFactRevision,
		})
	}
	return deltas, nil
}
...
func (reader *Reader) counterDeltaQuery(limit int) string {
  ...
  delta.delta_value,
  toUInt8(existing.existing_fact_count > 0) AS existing_fact_found,
...
LEFT JOIN (
 ...
 AND existing.site_id = delta.site_id
...
func (writer *Writer) InsertFacts(ctx context.Context, facts []energy.EnergyIntervalFact) error {
	...
	for _, fact := range ordered {
		...
		body.Write(encoded)
		body.WriteByte('\n')
		digest.Write([]byte(fmt.Sprintf("%s|%d|%s", fact.LogicalKey(), fact.FactRevision, optionalRebuildRunID(fact.RebuildRunID))))
		digest.Write([]byte{'\n'})
	}
...
func (writer *Writer) AppendRebuildEvent(ctx context.Context, event energy.RebuildEvent) error {
	...
	if err := event.ValidateForPersistence(); err != nil {
		return err
	}
...
var (
	...
	_ energy.RebuildEventSink = (*Writer)(nil)
)
