...
type CounterDelta struct {
	...
	DeltaValue                        *float64
	ExistingFactFound                 bool
	ExistingFactMeterBindingID        string
	ExistingFactPreviousObservationID string
	ExistingFactRevision              uint64
}
...
type RebuildEventSink interface {
	AppendRebuildEvent(context.Context, RebuildEvent) error
}
...
type ProjectorConfig struct {
	...
	FactSink        FactSink
	RebuildEvents   RebuildEventSink
	BatchSize       int
	Now             func() time.Time
}
...
func NewProjector(config ProjectorConfig) (*Projector, error) {
	...
	return &Projector{
		...
		rebuildEvents: config.RebuildEvents,
...
func (p *Projector) ProjectOnce(ctx context.Context) (int, error) {
	...
	for _, delta := range deltas {
		...
		fact.FactRevision, _, buildErr = correctionRevision(delta, resolution)
		if buildErr != nil {
			return 0, fmt.Errorf("classify energy fact correction for observation %s: %w", delta.CurrentObservationID, buildErr)
		}
		key := fact.LogicalKey()
